...
const StyledModalButton = ({ children, onClick, variant = "primary", className = "" }) => {
  const baseStyle = "inline-flex items-center justify-center px-5 py-2.5 text-sm font-semibold rounded-full transition-all duration-300 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white disabled:opacity-50 disabled:cursor-not-allowed transform hover:-translate-y-px active:translate-y-0";
  const variantStyles = {
    primary: "bg-green-600 text-white hover:bg-green-500 focus:ring-green-500 shadow-lg hover:shadow-xl bg-gradient-to-br from-green-500 to-teal-500",
    secondary: "bg-zinc-200 text-zinc-800 hover:bg-zinc-300 focus:ring-zinc-400 shadow-sm hover:shadow-md",
    danger: "bg-red-600 text-white hover:bg-red-500 focus:ring-red-500 shadow-lg hover:shadow-xl bg-gradient-to-br from-red-500 to-red-600"
  };
  return (
...
function GeneralModal({ isOpen, onClose, title, children, size = "md" }) {
  ...
  return (
    <div className="fixed inset-0 bg-zinc-900/50 backdrop-blur-md flex items-center justify-center z-[99999] p-4 print-hidden animate-fadeIn">
      <div className={`bg-white rounded-2xl shadow-2xl w-full ${sizeClasses[size] || sizeClasses.md} transition-all duration-300 ease-out animate-scaleIn border-t-4 border-green-500 overflow-hidden`}>
        <div className="flex justify-between items-center p-5 border-b border-zinc-200">
          ...
          {onClose && (
            <button onClick={onClose} className="text-zinc-400 hover:text-zinc-600 transition-colors p-1 rounded-full hover:bg-zinc-100">
...
function InputModal({ isOpen, onClose, onSubmit, title, message, inputLabel, initialValue = "", placeholder = "", submitText = "Submit", cancelText = "Cancel" }) {
  ...
  return (
    <GeneralModal isOpen={isOpen} onClose={onClose} title={title}>
      {message && <p className="text-sm text-zinc-600 mb-3">{message}</p>}
      <label htmlFor="input-modal-field" className="block text-sm font-medium text-zinc-700 mb-2">{inputLabel}</label>
      <input id="input-modal-field" type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder={placeholder} className="w-full px-3 py-2 border border-zinc-300 rounded-lg text-base focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-shadow shadow-sm" />
...
function CustomDropdown({ options, value, onChange, placeholder = "Select an option", label, disabled = false, idSuffix = "" }) {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      <button type="button" onClick={() => !disabled && setIsOpen(!isOpen)} disabled={disabled} className={`w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-shadow shadow-sm bg-white text-left flex justify-between items-center ${disabled ? "bg-zinc-100 cursor-not-allowed" : "cursor-pointer"}`}>
        <span className={selectedOption ? "text-zinc-800" : "text-zinc-500"}>{selectedOption ? selectedOption.label : placeholder}</span>
        <LucideIcons.ChevronDown className={`w-5 h-5 text-zinc-500 transition-transform ${isOpen ? "transform rotate-180" : ""}`} />
      </button>
      {isOpen && !disabled && (
        <ul className="absolute z-20 mt-1 w-full bg-white border border-zinc-200 rounded-lg max-h-60 overflow-auto py-1 text-sm shadow-lg animate-fadeInDown">
          {options.map((opt) => (
            <li key={`${opt.value}-${idSuffix}`} onClick={() => handleSelect(opt.value)} className={`px-4 py-2 hover:bg-green-50 cursor-pointer ${opt.value === value ? "bg-green-100 text-green-800 font-semibold" : "text-zinc-700"}`}>{opt.label}</li>
          ))}
        </ul>
      )}
    </div>
...
function Heading({ text = "Default Heading Title", onUpdate, isSelected, sizeClass, fontWeight, textAlign, textColor, isPreviewMode, isEditable, style }) {
  ...
  return (
    <div className={`p-2 ${!isPreviewMode ? `rounded-lg ${isSelected ? "" : "hover:ring-2 hover:ring-green-400/50"}`: ""}`} style={style}>
      <h1
        style={{ color: textColor || undefined }}
        className={`${sizeClass || "text-2xl"} ${fontWeight || "font-bold"} ${textAlign || "text-left"} ${!textColor ? "text-zinc-800" : ""} ${!isPreviewMode ? "focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white/80 p-1 -m-1 rounded-lg" : ""} transition-all`}
...
function TextBlock({ text = "Lorem ipsum dolor sit amet...", onUpdate, isSelected, sizeClass, fontWeight, textAlign, textColor, isPreviewMode, isEditable, style }) {
    ...
    return (
        <div className={`p-2 ${!isPreviewMode ? `rounded-lg ${isSelected ? "" : "hover:ring-2 hover:ring-green-400/50"}` : ""}`} style={style}>
            <p
                style={{ color: textColor || undefined }}
                className={`${sizeClass || "text-base"} ${fontWeight || "font-normal"} ${textAlign || "text-left"} ${!textColor ? "text-zinc-700" : ""} leading-relaxed ${!isPreviewMode ? "focus:outline-none focus:ring-2 focus:ring-green-500 focus:bg-white/80 p-1 -m-1 rounded-lg whitespace-pre-wrap" : "whitespace-pre-wrap"} transition-all`}
...
function AccordionElement({ title = "Accordion Title", content = "Accordion content...", onUpdate, isSelected, isPreviewMode, isEditable, style }) {
    ...
    return (
        <div className={`p-2 ${!isPreviewMode ? `rounded-lg ${isSelected ? "" : "hover:ring-2 hover:ring-green-400/50"}` : ""}`} style={style}>
            <div className={`bg-white rounded-lg shadow-sm border ${isSelected && !isPreviewMode ? "border-green-400" : "border-zinc-200"}`}>
                <h2>
                    <button type="button" onClick={toggleOpen} className="flex items-center justify-between w-full p-4 font-medium text-left text-zinc-800 hover:bg-green-50/50 transition-colors">
                ...
                <div className={`grid overflow-hidden transition-all duration-300 ease-in-out ${isOpen ? 'grid-rows-[1fr] opacity-100' : 'grid-rows-[0fr] opacity-0'}`}>
                    <div className="overflow-hidden">
                        <div className="p-4 border-t border-zinc-200 text-zinc-600">
...
function ImageElement({ src = img, alt = "Placeholder", width = "100%", height = "auto", borderRadius = "8px", boxShadow = "none", isSelected, isPreviewMode, style }) {
    ...
    return (
        <div className={`p-1 ${!isPreviewMode ? `rounded-lg ${isSelected ? "" : "hover:ring-2 hover:ring-green-400/50"}` : ""}`} style={style}>
...
function Spacer({ height = "20px", onUpdate, isSelected, isPreviewMode, style }) {
    return <div style={{ height, ...style }} className={`w-full transition-all ${!isPreviewMode && isSelected ? "bg-green-200/50" : !isPreviewMode ? "bg-transparent hover:bg-zinc-200/50" : ""}`}></div>;
}
...
function GoogleMapsPlaceholder({ address = "1600 Amphitheatre Parkway, Mountain View, CA", zoom = 14, onUpdate, isSelected, isPreviewMode, style }) {
    return (
        <div className={`p-3 rounded-xl ${!isPreviewMode ? `${isSelected ? "bg-green-500/10 ring-2 ring-green-500" : "bg-zinc-100 border border-zinc-200 hover:border-green-300"}` : "bg-zinc-100 border border-zinc-200"} aspect-video flex flex-col items-center justify-center text-center`} style={style}>
            <LucideIcons.MapPin className="h-12 w-12 text-zinc-400 mb-2" />
            <p className="text-sm font-medium text-zinc-600">{address}</p>
            <p className="text-xs text-zinc-500 mt-0.5">Maps Placeholder (Zoom: {zoom})</p>
        </div>
    );
}
...
function VideoElement({ videoType = "mp4", src, width = "100%", height = "auto", controls = true, autoplay = false, loop = false, muted = true, isSelected, isPreviewMode, style }) {
    ...
    const renderVideo = () => {
        if (!src) return (<div className="p-4 text-center text-zinc-500 aspect-video flex items-center justify-center bg-zinc-100 rounded-lg border border-zinc-200">Video source not configured.</div>);
        switch (videoType) {
    ...
    return (
        <div className={`p-1 ${!isPreviewMode ? `rounded-xl ${isSelected ? "" : "hover:ring-2 hover:ring-green-400/50"}` : ""}`} style={style}>
            {renderVideo()}
        </div>
    );
}
...
function InnerSectionComponentDisplay({ sectionData, onOpenStructureModal, onSelect, isSelected, onUpdateProps, onDelete, selectedItemId, isPreviewMode, isDraggable, style }) {
  ...
  if (!hasColumns) {
    return (
      <div onClick={(e) => { if (!isPreviewMode && isDraggable) { e.stopPropagation(); onSelect(sectionData.id, "element", ownPath); } }} className={`p-4 min-h-[80px] flex flex-col items-center justify-center ${!isPreviewMode ? `rounded-xl border-2 border-dashed ${isSelected ? "border-green-500 bg-green-50/80" : "border-zinc-300 bg-zinc-100/80 hover:border-green-400 hover:bg-green-50/50"} cursor-pointer transition-all` : ""}`} style={style}>
          <LucideIcons.Rows3 className="h-8 w-8 text-zinc-400 mb-2" />
          {!isPreviewMode && (
  ...
  return (
    <div onClick={(e) => { if (!isPreviewMode && isDraggable) { e.stopPropagation(); onSelect(sectionData.id, "element", ownPath); } }} className={`p-1 ${!isPreviewMode ? `border rounded-xl ${isSelected ? "border-green-500 bg-green-50/50" : "border-zinc-200 hover:border-green-300/70"}` : ""}`} style={style}>
...
function CardSliderElement({ slides = [], slidesPerView = 3, spaceBetween = 16, speed = 500, autoplay = false, autoplayDelay = 3000, loop = false, showNavigation = true, showPagination = true, cardBorderRadius = "12px", onUpdate, isSelected, isPreviewMode, onNavigate, isEditable, style }) {
  ...
  if (totalSlides === 0 && !isPreviewMode) {
      return (
          <div className={`p-4 min-h-[150px] flex flex-col items-center justify-center text-center border-2 border-dashed rounded-xl ${isSelected ? "border-green-500 bg-green-500/10" : "border-zinc-300 bg-zinc-100/80"}`} style={style}>
              <LucideIcons.GalleryHorizontalEnd className="h-12 w-12 text-zinc-400 mb-3" />
              <p className="text-sm font-medium text-zinc-600">Card Slider</p>
              <p className="text-xs text-zinc-500">Add slides in the Properties Panel.</p>
          </div>
      );
  }
  ...
  return (
      <div className={`p-2 relative ${!isPreviewMode && isSelected ? "rounded-xl bg-green-500/10" : ""}`} style={style}>
          ...
          {showNavigation && totalSlides > effectiveSlidesPerView && (<>
              <button onClick={handlePrev} disabled={!loop && currentIndex === 0} className="absolute top-1/2 left-0 transform -translate-y-1/2 -translate-x-3 bg-white/80 hover:bg-white text-zinc-700 p-2 rounded-full shadow-md z-10 disabled:opacity-40 disabled:cursor-not-allowed transition-all" aria-label="Previous slide"><LucideIcons.ChevronLeft className="w-6 h-6" /></button>
              <button onClick={handleNext} disabled={ !loop && currentIndex >= totalSlides - effectiveSlidesPerView } className="absolute top-1/2 right-0 transform -translate-y-1/2 translate-x-3 bg-white/80 hover:bg-white text-zinc-700 p-2 rounded-full shadow-md z-10 disabled:opacity-40 disabled:cursor-not-allowed transition-all" aria-label="Next slide"><LucideIcons.ChevronRight className="w-6 h-6" /></button>
          </>)}
          {showPagination && totalSlides > effectiveSlidesPerView && (
              <div className="absolute -bottom-2 left-1/2 transform -translate-x-1/2 flex space-x-2 p-2">
                  {Array.from({ length: loop ? totalSlides : Math.max(1, totalSlides - effectiveSlidesPerView + 1), }).map((_, idx) => (
                      <button key={idx} onClick={() => goToSlide(idx)} className={`w-2.5 h-2.5 rounded-full ${currentIndex === idx ? "bg-green-600 scale-125" : "bg-zinc-300 hover:bg-zinc-400"} transition-all`} aria-label={`Go to slide ${idx + 1}`} />
                  ))}
              </div>
          )}
      </div>
...
function NavbarElement({ logoType = "text", logoText = "MySite", logoSrc = img, links = [], rightContentType = "none", backgroundColor = "#ffffff", textColor = "#334155", linkColor = "#16a34a", isSelected, isPreviewMode, onUpdate, onNavigate, onSelect, onDelete, path, previewDevice, isDraggable, style }) {
    ...
    return (
        <div onClick={handleClick} className={`p-1 relative group ${!isPreviewMode && isSelected ? "rounded-xl bg-green-500/10" : !isPreviewMode && path ? "hover:ring-2 hover:ring-green-400/50 rounded-xl" : ""}`}>
...
function FooterElement({ copyrightText = `© ${new Date().getFullYear()} MySite. All rights reserved.`, links = [], backgroundColor = "#f1f5f9", textColor = "#64748b", linkColor = "#16a34a", isSelected, isPreviewMode, onUpdate, onNavigate, onSelect, onDelete, path, isDraggable, style }) {
    ...
    return (
        <div onClick={handleClick} className={`p-1 relative group ${!isPreviewMode && isSelected ? "rounded-xl bg-green-500/10" : !isPreviewMode && path ? "hover:ring-2 hover:ring-green-400/50 rounded-xl" : ""}`}>
            <footer style={footerStyle} className="px-6 py-10 text-center border-t border-zinc-200/80 rounded-xl">
            ...
            {!isPreviewMode && isSelected && onDelete && (
                <button onClick={(e) => { e.stopPropagation(); onDelete(); }} title="Remove Global Footer" className="absolute -top-2.5 -right-2.5 p-1 bg-red-600 text-white rounded-full hover:bg-red-700 hover:scale-110 transition-all w-7 h-7 flex items-center justify-center shadow-md z-30 print-hidden">
                    <LucideIcons.X className="w-4 h-4" strokeWidth={3} />
                </button>
            )}
        </div>
  ...
  return (
    <div className="space-y-3">
        {localLinks.map((link, index) => (
            <div key={link.id || index} className="p-3 border border-zinc-200 rounded-xl space-y-2 bg-zinc-50/70">
                <input type="text" placeholder={`${linkTypeLabel} Text`} value={link.text} onChange={(e) => handleLinkChange(index, "text", e.target.value)} className="w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500" />
                <input type="text" placeholder="URL (e.g. /page-slug or https://...)" value={link.url} onChange={(e) => handleLinkChange(index, "url", e.target.value)} className="w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500 mb-1.5" />
                ...
                <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center space-x-2">
                        <input type="checkbox" id={`${elementId}-link-${index}-target`} checked={link.target === "_blank"} onChange={(e) => handleLinkChange( index, "target", e.target.checked ? "_blank" : "_self" )} className="h-4 w-4 rounded border-zinc-300 text-green-600 focus:ring-green-500" />
                        <label htmlFor={`${elementId}-link-${index}-target`} className="text-xs text-zinc-600">Open in new tab</label>
                    </div>
    ...
    return (
      <div className="space-y-3">
          {localSlides.map((slide, index) => (
              <div key={slide.id || index} className="p-3 border border-zinc-200 rounded-xl space-y-2 bg-zinc-50/70">
                  <div className="flex justify-between items-center mb-1"><p className="text-sm font-medium text-zinc-700">Slide {index + 1}</p><button onClick={() => removeSlide(index)} className="text-red-500 hover:text-red-700 p-1 rounded-full hover:bg-red-100"><LucideIcons.Trash2 className="w-4 h-4" /></button></div>
                  {slide.imgSrc && (<img src={slide.imgSrc} alt={`Preview ${index}`} className="w-full h-24 object-contain rounded-lg border border-zinc-200 bg-white p-1 mb-2" />)}
                  <button onClick={() => slideImgInputRefs.current[index]?.current?.click()} className="w-full px-3 py-2 bg-zinc-200 text-zinc-700 text-sm font-medium rounded-lg hover:bg-zinc-300 transition-colors">Change Image</button>
                  ...
                  <input type="text" placeholder="Image URL" value={slide.imgSrc} onChange={(e) => handleSlideChange(index, "imgSrc", e.target.value)} className="w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500" />
                  <input type="text" placeholder="Heading" value={slide.heading} onChange={(e) => handleSlideChange(index, "heading", e.target.value)} className="w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500" />
                  <textarea placeholder="Text" value={slide.text} onChange={(e) => handleSlideChange(index, "text", e.target.value)} rows="3" className="w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500 whitespace-pre-wrap"></textarea>
                  <input type="text" placeholder="Link URL (optional)" value={slide.link || ""} onChange={(e) => handleSlideChange(index, "link", e.target.value)} className="w-full px-3 py-2 border border-zinc-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500" />
              </div>
          ))}
...
const AVAILABLE_ELEMENTS_CONFIG = [ 
    { id: "header", name: "Heading", component: "Heading", category: 'Basic', defaultProps: { text: "Powerful Headline Here", sizeClass: "text-4xl", fontWeight: "font-bold", textColor: "#18181b", textAlign: "text-left" } }, 
    { id: "textBlock", name: "Paragraph", component: "TextBlock", category: 'Basic', defaultProps: { text: "This is an engaging paragraph. You can edit this text to share more about your brand, services, or products.", sizeClass: "text-base", textColor: "#52525b", textAlign: "text-left" } }, 
...
function DraggableCanvasElement({ elementData, onUpdateProps, onDelete, onSelect, isSelected, onOpenStructureModal, parentColumnId, isPreviewMode, onNavigate, isDraggable, }) {
  ...
  return (
    <div ref={setNodeRef} style={style} onClick={handleClick} className={`relative group my-2 transition-all duration-150 ease-in-out ${!isPreviewMode ? `${isDragging ? "bg-green-50/80 shadow-2xl ring-2 ring-green-400 scale-[1.01]" : ""} ${isSelected ? "selected-element-style" : ""}` : ""}`}>
        {isDraggable && !isPreviewMode && (
          <div {...attributes} {...listeners} title="Drag element" className="absolute top-1/2 -left-4 transform -translate-y-1/2 p-2 cursor-grab bg-white hover:bg-green-600 text-zinc-500 hover:text-white rounded-full border border-zinc-300 hover:border-green-500 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus:opacity-100 z-20 print-hidden transition-all shadow-md">
        ...
        {!isPreviewMode && isSelected && isDraggable && (
          <button onClick={(e) => { e.stopPropagation(); onDelete(elementData.path); }} title="Delete element" className="absolute -top-3 -right-3 p-1 bg-red-600 text-white rounded-full hover:bg-red-700 hover:scale-110 transition-all w-7 h-7 flex items-center justify-center shadow-md z-20 print-hidden">
              <LucideIcons.X className="w-4 h-4" strokeWidth={3} />
          </button>
        )}
    </div>
  ...
  return (
    <div onClick={handleClick} style={columnStyle} className={`p-1.5 md:p-2 flex-shrink-0 transition-all ${!isPreviewMode && isDraggable ? 'cursor-pointer' : ''} ${!isPreviewMode && isSelected ? "outline outline-2 outline-offset-2 outline-green-500 rounded-2xl bg-green-50/70" : !isPreviewMode ? "hover:outline hover:outline-1 hover:outline-offset-1 hover:outline-green-300/70 rounded-2xl" : ""}`}>
        <SortableContext items={elementIds} strategy={verticalListSortingStrategy} disabled={isPreviewMode || !isDraggable}>
            <div ref={setDroppableRef} className={`min-h-[100px] p-2 rounded-xl transition-all ${!isPreviewMode ? `border ${isOver && isDraggable ? "bg-green-100/90 border-green-400 border-solid ring-2 ring-green-400" : "bg-white/30 border-zinc-200/90"} ${columnData.elements.length === 0 && !isOver ? "border-dashed flex items-center justify-center text-zinc-400/80 text-sm font-medium" : ""}` : ""}`}>
...
function SectionComponent({ sectionData, sectionIndex, onUpdateProps, onDelete, onSelect, selectedItemId, onOpenStructureModal, isPreviewMode, onNavigate, pageId, isDraggable, }) {
  ...
  const getSectionBaseBgClass = () => {
      ...
      const editModeBase = "shadow-lg rounded-2xl my-3";
  ...
  const sectionRootClasses = ["relative", "group", "transition-all", "duration-200", "ease-in-out", getSectionBaseBgClass(), !isPreviewMode && isSelected ? "selected-element-style" : "", !isPreviewMode && !isSelected ? "hover:ring-2 hover:ring-green-300/80 cursor-pointer" : ""].join(" ").replace(/\s+/g, " ").trim();
  ...
  return (
      <div ref={setNodeRef} style={effectiveBgStyle} className={sectionRootClasses} onClick={handleClick}>
          {!isPreviewMode && isDraggable && (
              <div {...attributes} {...listeners} title="Drag section" className="absolute top-4 -left-4 transform p-2 cursor-grab bg-white hover:bg-green-600 text-zinc-500 hover:text-white rounded-full border border-zinc-300 hover:border-green-500 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus:opacity-100 z-30 print-hidden transition-opacity shadow-md">
          ...
          {!isPreviewMode && isSelected && isDraggable && (
              <button onClick={(e) => { e.stopPropagation(); onDelete(sectionPath); }} title="Delete section" className="absolute -top-3 -right-3 p-1 bg-red-600 text-white rounded-full hover:bg-red-700 hover:scale-110 transition-all w-7 h-7 flex items-center justify-center shadow-md z-30 print-hidden">
    ...
    return (
        <div className="flex flex-col gap-4 items-center flex-shrink-0">
            ...
            <div className="relative">
                <div style={{ width: device.width }} className="bg-white shadow-2xl rounded-2xl border border-zinc-200 flex flex-col">
                    <div className="flex-1 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-300">
                        {globalNavbar && (
                            <header className="p-2 border-b border-zinc-200 shadow-sm z-10 flex-shrink-0 relative">
                        ...
                        <div className="p-4 bg-zinc-50/50 min-h-full">
                        <SortableContext items={sectionIds} strategy={verticalListSortingStrategy} disabled={isPreviewMode || !isDraggable}>
                            <div ref={setPageDroppableRef} className={`rounded-xl transition-all ${isOver && isDraggable ? "bg-green-100/80 ring-2 ring-green-400 ring-dashed" : ""} ${page.layout.length === 0 && !isOver ? "border-2 border-dashed border-zinc-300/80 min-h-[50vh]" : "border-transparent"}`}>
                                ...
                                {!isPreviewMode && page.layout.length === 0 && !isOver && isDraggable && (
                                    <div className="flex flex-col items-center justify-center h-full text-center py-24 select-none pointer-events-none">
                                        <LucideIcons.LayoutTemplate className="h-24 w-24 text-zinc-300/90 mb-5" strokeWidth={1} />
                                        <p className="text-zinc-500 text-xl font-medium mt-2">Your canvas is empty</p>
                                        <p className="text-zinc-400/90 text-base">Drag elements or add a new section.</p>
                                    </div>
                                )}
                            </div>
                        </SortableContext>
                        ...
                        {globalFooter && (
                            <footer className="p-2 border-t border-zinc-200 shadow-sm z-10 flex-shrink-0 relative bg-white">
        ...
        <GeneralModal isOpen={isOpen} onClose={onClose} title="Select a Structure" size="xl">
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-5 pt-2">
                {PREDEFINED_STRUCTURES.map((s) => (
                    <button key={s.id} onClick={() => { onSelectStructure(s.layout, context); onClose(); }} className="p-3 bg-zinc-50 rounded-xl hover:bg-white hover:ring-2 hover:ring-green-500 hover:shadow-lg hover:scale-[1.03] transition-all duration-200 flex flex-col items-center justify-start group focus:outline-none focus-visible:ring-2 focus-visible:ring-green-500">
                        <div className="flex w-full h-24 mb-3 space-x-2 items-stretch p-2 bg-white ring-1 ring-zinc-200 rounded-lg shadow-inner">
                            {s.layout.map((col, idx) => (
                                <div key={idx} className="bg-zinc-200 border border-zinc-300 group-hover:bg-green-200/60 group-hover:border-green-300 rounded-md transition-colors" style={{ flexBasis: col.width }}></div>
                            ))}
                        </div>
                        <span className="text-sm text-zinc-700 group-hover:text-green-800 text-center font-semibold">{s.name}</span>
                    </button>
                ))}
            </div>
        </GeneralModal>
...
function ElementPaletteItem({ config }) {
    ...
    return (
        <div ref={setNodeRef} {...listeners} {...attributes} style={style} className="flex flex-col items-center justify-center p-2 text-center bg-white border border-zinc-200 rounded-lg cursor-grab hover:bg-green-50 hover:border-green-300 hover:ring-1 hover:ring-green-300 transition-all group focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:ring-green-500">
            <div className="text-zinc-500 group-hover:text-green-600 mb-1.5 transition-colors">{IconToShow}</div>
            <span className="text-xs font-medium text-zinc-600 group-hover:text-green-800">{config.name}</span>
        </div>
    );
}
...
function AiModeView({ onBack, onAiSubmit, isAiLoading, aiChatHistory }) {
    ...
    return (
        <div className="p-3 flex flex-col h-full">
            <button onClick={onBack} className="flex items-center gap-2 text-sm text-zinc-600 hover:text-green-700 mb-4 p-1 rounded-md"><LucideIcons.ArrowLeft className="w-4 h-4" /> Back to Elements</button>
            <div className="relative">
                <textarea onKeyDown={handleKeyDown} placeholder="e.g., create a sleek corporate landing page..." className="w-full p-2 pr-8 border border-zinc-300 rounded-lg text-sm focus:ring-2 focus:ring-green-400 focus:border-green-400" rows="3" />
                <LucideIcons.CornerDownLeft className="absolute right-2 top-2 w-5 h-5 text-zinc-400" />
            </div>
            {isAiLoading && <div className="py-4"><AiLoader /></div>}
            <div className="mt-4 flex-1 overflow-y-auto space-y-3">
                {aiChatHistory.map(entry => (
                    <div key={entry.id} className="text-sm">
                        <p className="font-semibold text-zinc-700">{entry.prompt}</p>
...
function LayerNode({ node, type, path, depth, onSelect, selectedItemId }) {
    ...
    return (
        <div>
            <div onClick={handleSelect} className={`flex items-center p-1.5 rounded-lg cursor-pointer transition-colors ${isSelected ? 'bg-green-100 text-green-800' : 'hover:bg-zinc-100'}`}>
                <div style={{ paddingLeft: `${depth * 16}px` }} className="flex items-center flex-grow truncate">
                    {hasChildren ? (
                        <button onClick={handleToggle} className="p-0.5 mr-1 rounded-sm hover:bg-zinc-200">
                            <LucideIcons.ChevronRight className={`w-4 h-4 transition-transform ${isOpen ? 'rotate-90' : ''}`} />
                        </button>
                    ) : (
                        <span className="w-5 h-5 mr-1"></span>
                    )}
                    <div className="w-4 h-4 mr-2 text-zinc-500">{React.cloneElement(Icon, { className: 'w-full h-full' })}</div>
...
function LayersTreeView({ page, pagePath, onSelect, selectedItemId }) {
    if (!page) return <div className="p-4 text-sm text-zinc-500">No page selected.</div>;
    return (
...
const ElementAccordion = ({ title, children }) => {
    const [isOpen, setIsOpen] = useState(true);
    return (
        <div>
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center p-2 rounded-lg hover:bg-zinc-200/50 transition-colors">
                <h3 className="text-xs font-semibold uppercase text-zinc-500 tracking-wider">{title}</h3>
                <LucideIcons.ChevronDown className={`w-5 h-5 text-zinc-500 transition-transform duration-300 ${isOpen ? 'rotate-180' : ''}`}/>
            </button>
...
function LeftPanel({ isOpen, onClose, onAddTopLevelSection, pages, activePageId, onAddPage, onSelectPage, onRenamePage, onDeletePage, onAiSubmit, isAiLoading, aiChatHistory, onSwitchToAiMode, onSelect, selectedItem }) {
  ...
  const TabButton = ({ tabName, icon, label }) => (
    <div className="flex-1 relative">
        <button onClick={() => setActiveTab(tabName)} className={`w-full p-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === tabName ? "text-green-600" : "text-zinc-500 hover:text-zinc-800"}`}>
            {icon} {label}
        </button>
        {activeTab === tabName && <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-green-500 rounded-full animate-fadeIn"></div>}
    </div>
  );
  ...
  return (
    <aside className={`absolute top-0 left-0 h-full w-80 bg-zinc-50/80 border-r border-zinc-200/80 backdrop-blur-xl flex-shrink-0 flex flex-col print-hidden transition-transform duration-300 ease-in-out z-40 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-center p-3 border-b border-zinc-200/80 h-[60px]">
            <h2 className="text-lg font-semibold text-zinc-800 px-1.5">Add Content</h2>
            <button onClick={onClose} className="p-2 rounded-full text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800">
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            {activeTab === 'insert' && !isAiMode && (
                <div className="p-3 space-y-5">
                    <div className="relative"><LucideIcons.Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-zinc-400" /><input type="text" placeholder="Search elements..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2 bg-white border border-zinc-300 rounded-lg text-sm focus:ring-1 focus:ring-green-500" /></div>
                    <button onClick={handleAiClick} className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-green-100 text-green-700 font-semibold rounded-lg hover:bg-green-200 transition-colors border border-green-200 hover:border-green-300 relative group">
                        ...
                        <LucideIcons.Sparkles className="w-4 h-4 relative"/> 
                        <span className="relative">Build with AI</span>
                    </button>
                    <ElementAccordion title="Structure">
                        <div className="p-1">
                            <button onClick={onAddTopLevelSection} className="w-full flex items-center justify-center gap-3 p-3 text-left bg-white border border-zinc-200 rounded-lg cursor-pointer hover:bg-green-50 hover:border-green-300 hover:ring-1 hover:ring-green-300 transition-all group focus:outline-none focus-visible:ring-1 focus-visible:ring-green-500">
                                <LucideIcons.PlusSquare className="w-5 h-5 text-zinc-500 group-hover:text-green-600 transition-colors" />
                                <span className="text-sm font-medium text-zinc-700 group-hover:text-green-800">Add Section</span>
                            </button>
                        </div>
                    </ElementAccordion>
                    {Object.entries(categorizedElements).map(([category, elements]) => (
                        elements.length > 0 && <ElementAccordion key={category} title={category}>
                            <div className="grid grid-cols-2 gap-2">{elements.map((elConf) => (<ElementPaletteItem key={elConf.id} config={elConf} />))}</div>
                        </ElementAccordion>
                    ))}
                    {globalElements.length > 0 && (
                        <ElementAccordion title="Global">
                            <div className="grid grid-cols-2 gap-2">{globalElements.map((elConf) => (<ElementPaletteItem key={elConf.id} config={elConf} />))}</div>
                        </ElementAccordion>
                    )}
                </div>
            )}
                    ...
                    <ul className="space-y-1">
                        {Object.values(pages).map((page) => (
                            <li key={page.id} className="group">
                                <button onClick={() => onSelectPage(page.id)} className={`w-full text-left px-3 py-2 text-sm rounded-lg transition-all duration-150 flex items-center justify-between ${activePageId === page.id ? "bg-green-600 text-white font-semibold shadow-sm" : "text-zinc-600 hover:bg-zinc-100 hover:text-zinc-900"}`}>
...
function DebouncedTextInput({ label, type, initialValue, onCommit, ...props }) {
  ...
  return (
    <div>
        {label && <label className="block text-xs font-medium text-zinc-600 mb-1.5">{label}</label>}
        <InputComponent type={type} value={value || ''} onChange={e => setValue(e.target.value)} onBlur={handleBlur} className="w-full px-3 py-1.5 border border-zinc-300 rounded-lg text-sm focus:ring-1 focus:ring-green-500 focus:border-green-500" {...props} />
    </div>
  );
}
...
function RightSidebar({ selectedItemData, onUpdateSelectedProps, pages, activePageId, onRenamePage, onAddGlobalElement, comments, onUpdateComment, onDeleteComment }) {
  ...
  const PropertyGroup = ({ title, children, defaultOpen = true }) => {
      ...
      return (
        <div className="border-b border-zinc-200/80 pb-4 mb-4">
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center py-1">
                <h3 className="text-xs font-semibold uppercase text-zinc-500 tracking-wider">{title}</h3>
                <LucideIcons.ChevronDown className={`w-4 h-4 text-zinc-400 transition-transform ${isOpen ? 'rotate-180' : ''}`} />
            </button>
  ...
  const ColorInput = ({ label, value, onChange }) => (
    <div className="flex justify-between items-center">
        ...
        <div className="flex items-center gap-2">
            <input type="text" value={value || '#000000'} onChange={e => onChange(e.target.value)} className="w-24 text-sm border-zinc-300 rounded-md px-2 py-1 focus:ring-1 focus:ring-green-500" />
            <div className="relative w-9 h-7 rounded-md overflow-hidden border border-zinc-300 shadow-sm">
  ...
  const StyledSlider = ({ label, value, onChange, min = 0, max = 200, step = 1, unit = "px" }) => {
      ...
      return (
        <div>
            <div className="flex justify-between items-center mb-1"><label className="text-xs font-medium text-zinc-600">{label}</label><span className="text-xs text-zinc-500">{`${displayValue}${unit}`}</span></div>
  ...
  const ToggleSwitch = ({ label, checked, onChange }) => (
    <div className="flex justify-between items-center">
        <label className="text-sm font-medium text-zinc-700">{label}</label>
        <button onClick={() => onChange(!checked)} className={`relative inline-flex items-center h-6 rounded-full w-11 transition-colors ${checked ? 'bg-green-600' : 'bg-zinc-300'}`}><span className={`inline-block w-4 h-4 transform bg-white rounded-full transition-transform ${checked ? 'translate-x-6' : 'translate-x-1'}`} /></button>
    </div>
  );
  const AlignmentButtons = ({ value, onChange }) => (
    <div>
        <label className="block text-xs font-medium text-zinc-600 mb-1.5">Alignment</label>
        <div className="flex items-center bg-zinc-200 rounded-lg p-0.5">{textAlignOptions.map(opt => (<button key={opt.value} onClick={() => onChange(opt.value)} title={opt.label} className={`flex-1 p-1.5 rounded-md transition-all text-zinc-600 ${value === opt.value ? 'bg-white text-green-700 shadow-sm' : 'hover:bg-zinc-200/50'}`}>{opt.icon}</button>))}</div>
    </div>
  );
  const TabButton = ({ tabName, icon, label }) => (
    <div className="flex-1 relative">
  ...
  const renderPropertiesPanel = () => {
    if (!selectedItemData || !selectedItemData.itemType) {
        return ( <div className="flex flex-col items-center justify-center h-full text-center p-4"><LucideIcons.Pointer className="w-16 h-16 text-zinc-300 mb-4"/><h3 className="font-semibold text-zinc-700">Nothing Selected</h3><p className="text-sm text-zinc-500 mt-1">Click an element on the canvas to edit.</p></div> );
    }
    if (selectedItemData.itemType === 'page') {
      const currentPage = pages[activePageId];
      return ( <> <div className="flex justify-between items-center p-4 border-b border-zinc-200/80 h-[60px]"><h2 className="text-lg font-semibold text-zinc-800">Page Settings</h2></div><div className="p-4 space-y-4"><DebouncedTextInput label="Page Name" initialValue={currentPage?.name || ""} onCommit={onRenamePage} key={activePageId}/></div> </> );
    }
    ...
    return (<> <div className="flex justify-between items-center p-4 border-b border-zinc-200/80 h-[60px]"><h2 className="text-lg font-semibold text-zinc-800 capitalize">{config?.name || itemType || 'Properties'}</h2></div><div className="overflow-y-auto flex-grow text-sm p-4"> {(() => {
        switch(itemType) {
            ...
            default: return <p className="text-sm text-zinc-500 text-center py-8">No properties to edit for this '{itemType}'.</p>;
        }
    })()}</div></>);
  };
  const renderCommentsPanel = () => {
    const pageComments = comments[activePageId] || [];
    const IconForFrame = ({frame}) => { const DeviceIcon = DEVICE_FRAMES_CONFIG.find(d => d.name === frame)?.icon || LucideIcons.HelpCircle; return <DeviceIcon className="w-4 h-4 text-zinc-500"/> };
    return ( <> <div className="flex justify-between items-center p-4 border-b border-zinc-200/80 h-[60px]"><h2 className="text-lg font-semibold text-zinc-800">Comments</h2></div> <div className="overflow-y-auto flex-grow text-sm p-4 space-y-3"> {pageComments.length === 0 ? ( <div className="text-center py-10 text-zinc-500"><LucideIcons.MessageSquarePlus className="mx-auto w-12 h-12 text-zinc-300 mb-2"/><p>No comments on this page.</p><p className="text-xs">Use the Comment tool to add notes.</p></div> ) : ( pageComments.map(comment => ( <div key={comment.id} className="p-3 bg-zinc-100 rounded-lg"><div className="flex justify-between items-start"><textarea value={comment.text} onChange={(e) => onUpdateComment(activePageId, comment.id, e.target.value)} className="w-full bg-transparent border-0 focus:ring-0 p-0 text-sm text-zinc-800 resize-none" rows={2}/> <button onClick={() => onDeleteComment(activePageId, comment.id)} className="ml-2 p-1 text-red-500 hover:bg-red-100 rounded-full"><LucideIcons.Trash2 className="w-4 h-4"/></button> </div> <div className="text-xs text-zinc-500 mt-2 flex items-center gap-2"> <IconForFrame frame={comment.frame} /> <span>on {comment.frame} view</span> </div></div> )) )} </div> </> )
  }

  return (
    <aside className="h-full w-80 bg-white/80 border-l border-zinc-200/80 backdrop-blur-xl flex flex-col print-hidden">
        <div className="flex border-b border-zinc-200/80">
...
function AiCanvasLoader() {
    return (
        <div className="absolute inset-0 bg-gradient-to-br from-green-50/70 via-teal-100/60 to-green-100/80 flex flex-col items-center justify-center z-[1000] overflow-hidden">
            ...
            <p className="text-zinc-800 text-lg font-semibold mt-8 tracking-wider animate-pulse">Building with AI...</p>
...
export function PagePreviewRenderer({ pageLayout, globalNavbar, globalFooter, onNavigate, activePageId }) {
    ...
    return (
        <div className="flex-1 overflow-hidden bg-zinc-100 flex flex-col items-center p-4 gap-4">
            <div className="bg-white rounded-full p-1.5 flex items-center justify-center gap-2 text-sm font-medium text-zinc-600 shadow-md">
                {PREVIEW_DEVICES.map(d => (<button key={d.name} onClick={() => setDevice(d)} className={`px-4 py-2 rounded-full flex items-center gap-2 transition-colors ${device.name === d.name ? 'bg-green-600 text-white' : 'hover:bg-zinc-100'}`}><d.icon className="w-5 h-5" /> {d.name}</button>))}
            </div>
            <div className="flex-1 w-full flex items-center justify-center overflow-hidden">
                <div style={{ maxWidth: device.width, width: '100%', height: '100%' }} className="bg-white shadow-2xl mx-auto transition-all duration-300 rounded-2xl">
                    <div className="w-full h-full overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-400 scrollbar-track-zinc-100">
...
function CanvasToolbar({ selectedItem, zoom, onZoomChange, onSelect, pages, activeTool, onToolChange }) {
    ...
    return (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/60 backdrop-blur-xl h-12 p-1.5 rounded-full border border-zinc-200/80 flex items-center justify-between text-sm z-30 shadow-lg shadow-black/5">
            <div className="flex items-center gap-1 pl-1">
                <button onClick={() => onToolChange('select')} title="Select Tool (V)" className={`p-2 rounded-full transition-all ${activeTool === 'select' ? 'bg-green-100 text-green-700 shadow-sm' : 'hover:bg-zinc-200/80 text-zinc-600'}`}><LucideIcons.MousePointer2 className="w-5 h-5"/></button>
                <button onClick={() => onToolChange('hand')} title="Hand Tool (H)" className={`p-2 rounded-full transition-all ${activeTool === 'hand' ? 'bg-green-100 text-green-700 shadow-sm' : 'hover:bg-zinc-200/80 text-zinc-600'}`}><LucideIcons.Hand className="w-5 h-5"/></button>
                <button onClick={() => onToolChange('comment')} title="Comment (C)" className={`p-2 rounded-full transition-all ${activeTool === 'comment' ? 'bg-green-100 text-green-700 shadow-sm' : 'hover:bg-zinc-200/80 text-zinc-600'}`}><LucideIcons.MessageSquarePlus className="w-5 h-5"/></button>
            </div>
            <div className="h-6 w-px bg-zinc-300 mx-2"></div>
            <div className="flex items-center gap-1 text-zinc-500 px-2 max-w-md overflow-x-auto">
                {breadcrumbs.map((crumb, index) => (
                    <React.Fragment key={index}>
                        <button onClick={() => onSelect(crumb.id, crumb.type, crumb.path)} className={`px-2 py-1 rounded-md transition-colors whitespace-nowrap ${index === breadcrumbs.length - 1 ? 'text-zinc-800 font-semibold bg-zinc-200/60' : 'hover:bg-zinc-200/50'}`}>{crumb.name}</button>
            ...
            <div className="flex items-center gap-2 px-2">
                <button onClick={() => onZoomChange(Math.max(0.1, zoom - 0.1))} className="p-1.5 hover:bg-zinc-200 rounded-md text-zinc-600"><LucideIcons.Minus className="w-4 h-4" /></button>
                <span className="w-12 text-center font-medium text-zinc-700">{Math.round(zoom * 100)}%</span>
                <button onClick={() => onZoomChange(zoom + 0.1)} className="p-1.5 hover:bg-zinc-200 rounded-md text-zinc-600"><LucideIcons.Plus className="w-4 h-4" /></button>
            </div>
        </div>
    );
}
...
function TopBar({ onSave, onTogglePreview, isPreviewMode, onToggleLeftPanel, onToggleFullscreen, isFullscreen }) {
    return (
        <header className="bg-white/95 backdrop-blur-sm h-[60px] p-2.5 border-b border-zinc-200/80 shadow-sm print-hidden flex justify-between items-center px-4 z-[50] relative">
            <div className="flex items-center gap-3">
                {!isPreviewMode && (
                    <button onClick={onToggleLeftPanel} className="p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800 transition-colors" title="Toggle Content Panel">
                        <LucideIcons.PanelLeft className="w-6 h-6" />
                    </button>
                )}
                <a href="#" className="flex items-center gap-2.5 text-zinc-800 text-lg">
                    <LucideIcons.Feather className="w-6 h-6 text-green-500" />
                    <span className="hidden sm:inline font-semibold">Page<span className="font-light text-green-500">Builder</span></span>
                </a>
            </div>
            
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                <div className="relative flex items-center p-1 bg-zinc-200/70 rounded-full border border-zinc-300/50 shadow-inner">
                     ...
                     <button onClick={() => isPreviewMode && onTogglePreview()} className={`relative z-10 px-4 py-1.5 rounded-full flex items-center gap-2 transition-colors duration-300 text-sm font-medium ${!isPreviewMode ? "text-green-700" : "text-zinc-500"}`}>
            ...
            <div className="flex items-center gap-2">
                {!isPreviewMode && (
                 ...
                 {!isPreviewMode && (
                    <button onClick={onToggleFullscreen} className="p-2 rounded-lg text-zinc-500 hover:bg-zinc-100 hover:text-zinc-800 transition-colors" title={isFullscreen ? "Exit Fullscreen" : "Fullscreen"}>
...
function htmlToBuilderJson(htmlString) {
    const parser = new DOMParser(); const doc = parser.parseFromString(htmlString, 'text/html'); const sections = [];
    const classToPropMap = { 'text-left': { textAlign: 'text-left' }, 'text-center': { textAlign: 'text-center' }, 'text-right': { textAlign: 'text-right' }, 'font-bold': { fontWeight: 'font-bold' }, 'font-extrabold': { fontWeight: 'font-bold' }, 'font-semibold': { fontWeight: 'font-semibold' }, 'font-medium': { fontWeight: 'font-medium' }, 'font-normal': { fontWeight: 'font-normal' }, 'font-light': { fontWeight: 'font-light' }, 'text-xs': { sizeClass: 'text-xs' }, 'text-sm': { sizeClass: 'text-sm' }, 'text-base': { sizeClass: 'text-base' }, 'text-lg': { sizeClass: 'text-lg' }, 'text-xl': { sizeClass: 'text-xl' }, 'text-2xl': { sizeClass: 'text-2xl' }, 'text-3xl': { sizeClass: 'text-3xl' }, 'text-4xl': { sizeClass: 'text-4xl' }, 'text-5xl': { sizeClass: 'text-5xl' } };
    
    function parseNode(node) {
        if (node.nodeType !== Node.ELEMENT_NODE) return null;
        let element = null;
        if (node.tagName.match(/^H[1-6]$/)) { element = { type: 'header', props: { text: node.innerHTML.trim() } }; }
        else if (node.tagName === 'P') { element = { type: 'textBlock', props: { text: node.innerHTML.trim() } }; }
        else if (node.tagName === 'BUTTON' || (node.tagName === 'A' && node.classList.contains('button'))) { element = { type: 'button', props: { buttonText: node.innerHTML.trim() } }; }
        else if (node.tagName === 'IMG') { element = { type: 'image', props: { src: node.src, alt: node.alt }}; }
        else if (node.tagName === 'HR') { element = { type: 'divider', props: {} }; }
        ...
        if (element) {
            element.id = generateId(element.type); const config = AVAILABLE_ELEMENTS_CONFIG.find(c => c.id === element.type);
            if (config) { element.props = { ...config.defaultProps, ...element.props }; }
            node.classList.forEach(cls => { if(classToPropMap[cls]) { Object.assign(element.props, classToPropMap[cls]); } });
            return element;
        } return null;
    }
    ...
    function parseContainer(containerEl) {
        ...
        containerEl.childNodes.forEach(childNode => {
            if (childNode.nodeType !== Node.ELEMENT_NODE || ['SCRIPT', 'STYLE', 'HEADER', 'FOOTER'].includes(childNode.tagName) || (childNode.id && childNode.id.includes('modal'))) { return; }
            
            if (childNode.classList.contains('accordion')) {
                const parsedEl = parseNode(childNode);
                if (parsedEl) { elements.push(parsedEl); }
                return;
            }

            const parsedEl = parseNode(childNode); if (parsedEl) { elements.push(parsedEl); } else if (childNode.children.length > 0) { elements.push(...parseContainer(childNode)); }
        });
        return elements;
    }

    const body = doc.querySelector('body'); const main = doc.querySelector('main'); const contentRoot = main || body;
    if (contentRoot) {
        contentRoot.querySelectorAll(':scope > section').forEach(sectionEl => {
            ...
            if (layoutContainers.length > 0 && Array.from(layoutContainers[0].children).every(child => child.tagName === 'DIV' || child.tagName === 'ARTICLE')) {
                const container = layoutContainers[0]; const cols = Array.from(container.children); const colWidth = 100 / (cols.length || 1);
                cols.forEach(colEl => { const newCol = { id: generateId('col'), type: 'column', props: { width: `${colWidth}%`}, elements: parseContainer(colEl) }; newSection.columns.push(newCol); });
            } else { const newColumn = { id: generateId('col'), type: 'column', props: { width: '100%' }, elements: parseContainer(sectionEl) }; newSection.columns.push(newColumn); }
...
export default function ElementBuilderPage({ onExternalSave, initialBuilderState }) {
  ...
  const handleAiSubmit = async (prompt) => {
    ...
    try {
        const pageResponse = await fetch('http://104.219.251.122:8000/generate-page', { method: 'POST', headers: { 'accept': 'application/json', 'Content-Type': 'application/json' }, body: JSON.stringify({ session_id: aiSessionId.current, prompt: prompt, as_file: false }) });
        if (!pageResponse.ok) throw new Error(`Generation API error: ${pageResponse.statusText}`);
        const apiResult = await pageResponse.json();
        if (apiResult.html) { const newLayout = htmlToBuilderJson(apiResult.html); updateLayoutForPage(activePageId, () => newLayout); setAiChatHistory(prev => prev.map(entry => entry.id === historyId ? {...entry, status: 'success'} : entry)); } 
        else { throw new Error("Invalid response structure from AI. 'html' key not found."); }
    } catch (error) {
  ...
  return (
    <DndContext sensors={sensors} collisionDetection={closestCenter} onDragStart={handleDragStart} onDragEnd={handleDragEnd} disabled={isPreviewMode || activeTool !== 'select'}>
        <div ref={builderRef} className="h-screen bg-white antialiased flex flex-col relative">
            <style>{`.selected-element-style { position: relative; } .selected-element-style::after { content: ''; position: absolute; inset: -3px; border: 2px solid #10b981; border-radius: 1.25rem; animation: pulse-ring 1.8s infinite ease-in-out; pointer-events: none; z-index: 10; box-shadow: 0 0 15px rgba(16, 185, 129, 0.5); } @keyframes pulse-ring { 0%, 100% { transform: scale(1); opacity: 0.6; } 50% { transform: scale(1.02); opacity: 1; } } .custom-slider { -webkit-appearance: none; appearance: none; width: 100%; height: 8px; background: #e5e7eb; border-radius: 9999px; outline: none; opacity: 0.9; transition: opacity .2s; } .custom-slider:hover { opacity: 1; } .custom-slider::-webkit-slider-thumb { -webkit-appearance: none; appearance: none; width: 18px; height: 18px; background: #10b981; border-radius: 50%; cursor: pointer; border: 2px solid white; box-shadow: 0 0 5px rgba(0,0,0,0.2); } .custom-slider::-moz-range-thumb { width: 18px; height: 18px; background: #10b981; border-radius: 50%; cursor: pointer; border: 2px solid white; box-shadow: 0 0 5px rgba(0,0,0,0.2); } `}</style>
