...
const StyledModalButton = ({ children, onClick, variant = "primary", className = "" }) => {
  const baseStyle = "inline-flex items-center justify-center px-4 py-2 text-sm font-semibold rounded-full transition-all duration-200 ease-in-out focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-white disabled:opacity-50 disabled:cursor-not-allowed transform";
  const variantStyles = {
    primary: "bg-green-600 text-white hover:bg-green-700 focus:ring-green-500 shadow-md hover:shadow-lg hover:-translate-y-px",
    secondary: "bg-slate-200 text-slate-800 hover:bg-slate-300 focus:ring-slate-400 shadow-sm hover:shadow-md hover:-translate-y-px",
    danger: "bg-red-600 text-white hover:bg-red-700 focus:ring-red-500 shadow-md hover:shadow-lg hover:-translate-y-px"
  };
  return (
...
function InputModal({ isOpen, onClose, onSubmit, title, message, inputLabel, initialValue = "", placeholder = "", submitText = "Submit", cancelText = "Cancel" }) {
  ...
  return (
    <GeneralModal isOpen={isOpen} onClose={onClose} title={title}>
      ...
      <input id="input-modal-field" type="text" value={inputValue} onChange={(e) => setInputValue(e.target.value)} placeholder={placeholder} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-base focus:ring-2 focus:ring-green-500 focus:border-green-500 transition-shadow shadow-sm" />
...
function CustomDropdown({ options, value, onChange, placeholder = "Select an option", label, disabled = false, idSuffix = "" }) {
  ...
  return (
    <div className="relative w-full" ref={dropdownRef}>
      ...
      {isOpen && !disabled && (
        <ul className="absolute z-20 mt-1 w-full bg-white border border-slate-300 rounded-lg max-h-60 overflow-auto py-1 text-sm shadow-lg">
          {options.map((opt) => (
...
const textSizeOptions = [
  { label: "Tiny", value: "text-xs" }, { label: "Small", value: "text-sm" }, { label: "Base", value: "text-base" },
  { label: "Large", value: "text-lg" }, { label: "XL", value: "text-xl" }, { label: "2XL", value: "text-2xl" },
];
const fontWeightOptions = [
  { label: "Light", value: "font-light" }, { label: "Normal", value: "font-normal" }, { label: "Medium", value: "font-medium" },
  { label: "Semibold", value: "font-semibold" }, { label: "Bold", value: "font-bold" },
];
const textAlignOptions = [
  { label: "Left", value: "text-left", icon: <LucideIcons.AlignLeft className="w-5 h-5" /> },
  { label: "Center", value: "text-center", icon: <LucideIcons.AlignCenter className="w-5 h-5" /> },
  { label: "Right", value: "text-right", icon: <LucideIcons.AlignRight className="w-5 h-5" /> },
];
const PREDEFINED_STRUCTURES = [
  ...
  { name: "Left Sidebar", id: "leftsidebar", layout: [{ width: "30%" }, { width: "70%" }] },
  { name: "Right Sidebar", id: "rightsidebar", layout: [{ width: "70%" }, { width: "30%" }] },
];
...
function VideoElement({ videoType = "mp4", src, width = "100%", height = "auto", controls = true, autoplay = false, loop = false, muted = true, isSelected, isPreviewMode, style }) {
    ...
    const renderVideo = () => {
        ...
        switch (videoType) {
            case "youtube": return (<iframe src={getYouTubeEmbedUrl(src)} style={{ width: effectiveWidth, height: effectiveHeight }} frameBorder="0" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen title="YouTube Video" className="block mx-auto rounded-lg"></iframe>);
            case "vimeo": return (<iframe src={getVimeoEmbedUrl(src)} style={{ width: effectiveWidth, height: effectiveHeight }} frameBorder="0" allow="autoplay; fullscreen; picture-in-picture" allowFullScreen title="Vimeo Video" className="block mx-auto rounded-lg"></iframe>);
            case "mp4": default: return (<video src={src} style={{ width: effectiveWidth, height: effectiveHeight }} controls={controls} autoPlay={autoplay} loop={loop} muted={muted} playsInline className="block mx-auto bg-black rounded-lg"></video>);
        }
    };
    return (
  ...
  return (
      <div className={`p-2 relative ${!isPreviewMode && isSelected ? "rounded-xl bg-green-500/10" : ""}`} style={style}>
              ...
              <div ref={sliderWrapperRef} className="flex transition-transform duration-500 ease-in-out" style={{ transform: `translateX(-${currentIndex * (100 / effectiveSlidesPerView)}%)`, transitionDuration: `${speed}ms` }}>
                  {slides.map((slide, index) => (
                      <div key={slide.id || index} className="flex-shrink-0 w-full" style={{ width: `${100 / effectiveSlidesPerView}%`, paddingLeft: `${spaceBetween / 2}px`, paddingRight: `${spaceBetween / 2}px` }}>
                          <div className="bg-white p-4 flex flex-col h-full shadow-md" style={{ borderRadius: cardBorderRadius }}>
          ...
          {showNavigation && totalSlides > effectiveSlidesPerView && (<>
              <button onClick={handlePrev} disabled={!loop && currentIndex === 0} className="absolute top-1/2 left-0 transform -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white p-2 rounded-full z-10 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity" aria-label="Previous slide"><LucideIcons.ChevronLeft className="w-6 h-6" /></button>
              <button onClick={handleNext} disabled={ !loop && currentIndex >= totalSlides - effectiveSlidesPerView } className="absolute top-1/2 right-0 transform -translate-y-1/2 bg-black/40 hover:bg-black/60 text-white p-2 rounded-full z-10 disabled:opacity-50 disabled:cursor-not-allowed transition-opacity" aria-label="Next slide"><LucideIcons.ChevronRight className="w-6 h-6" /></button>
          </>)}
...
function NavbarElement({ logoType = "text", logoText = "MySite", logoSrc = img, links = [], rightContentType = "none", backgroundColor = "#ffffff", textColor = "#334155", linkColor = "#16a34a", isSelected, isPreviewMode, onUpdate, onNavigate, onSelect, onDelete, path, previewDevice, isDraggable, style }) {
    ...
    return (
        <div onClick={handleClick} className={`p-1 relative group ${!isPreviewMode && isSelected ? "rounded-xl bg-green-500/10" : !isPreviewMode && path ? "hover:ring-1 hover:ring-green-400/70 rounded-xl" : ""}`}>
...
function FooterElement({ copyrightText = `© ${new Date().getFullYear()} MySite. All rights reserved.`, links = [], backgroundColor = "#f1f5f9", textColor = "#64748b", linkColor = "#16a34a", isSelected, isPreviewMode, onUpdate, onNavigate, onSelect, onDelete, path, isDraggable, style }) {
    ...
    const handleClick = (e) => { if (!isPreviewMode && onSelect && isDraggable) { e.stopPropagation(); onSelect(); } };
    return (
        <div onClick={handleClick} className={`p-1 relative group ${!isPreviewMode && isSelected ? "rounded-xl bg-green-500/10" : !isPreviewMode && path ? "hover:ring-1 hover:ring-green-300/70 rounded-xl" : ""}`}>
  ...
  return (
    <div className="space-y-3">
        {localLinks.map((link, index) => (
            <div key={link.id || index} className="p-3 border border-slate-200 rounded-xl space-y-2 bg-slate-50/70">
                ...
                <input type="text" placeholder="URL (e.g. /page-slug or https://...)" value={link.url} onChange={(e) => handleLinkChange(index, "url", e.target.value)} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-green-500 focus:border-green-500 mb-1.5" />
    ...
    return (
      <div className="space-y-3">
          {localSlides.map((slide, index) => (
              <div key={slide.id || index} className="p-3 border border-slate-200 rounded-xl space-y-2 bg-slate-50/70">
...
const ALL_ELEMENT_TYPES = {
    Heading, TextBlock, ImageElement, ButtonElement, Divider, Spacer, IconElement,
    GoogleMapsPlaceholder, VideoElement, InnerSectionComponentDisplay,
...
function DraggableCanvasElement({ elementData, onUpdateProps, onDelete, onSelect, isSelected, onOpenStructureModal, parentColumnId, isPreviewMode, onNavigate, isDraggable, }) {
  ...
  return (
    <div ref={setNodeRef} style={style} onClick={handleClick} className={`relative group my-2 transition-all duration-150 ease-in-out ${!isPreviewMode ? `${isDragging ? "bg-green-50/80 shadow-xl ring-2 ring-green-400 scale-[1.01]" : ""} ${isSelected ? "selected-element-style" : ""}` : ""}`}>
        {isDraggable && !isPreviewMode && (
          <div {...attributes} {...listeners} title="Drag element" className="absolute top-1/2 -left-4 transform -translate-y-1/2 p-2 cursor-grab bg-white hover:bg-green-600 text-slate-500 hover:text-white rounded-full border border-slate-300 hover:border-green-500 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus:opacity-100 z-20 print-hidden transition-all">
  ...
  return (
      <div ref={setNodeRef} style={effectiveBgStyle} className={sectionRootClasses} onClick={handleClick}>
          {!isPreviewMode && isDraggable && (
              <div {...attributes} {...listeners} title="Drag section" className="absolute top-2.5 -left-4 transform p-2 cursor-grab bg-white hover:bg-green-600 text-slate-500 hover:text-white rounded-full border border-slate-300 hover:border-green-500 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 focus:opacity-100 z-30 print-hidden transition-opacity">
        ...
        <div className="flex flex-col gap-4 items-center flex-shrink-0">
            ...
            <div className="relative">
                ...
                {comments.map((comment, index) => (
                    <div key={comment.id} style={{ top: `${comment.position.y}px`, left: `${comment.position.x}px`}} className="absolute w-8 h-8 bg-yellow-400 rounded-full flex items-center justify-center text-black font-bold text-sm border-2 border-white cursor-pointer z-50 transform -translate-x-1/2 -translate-y-1/2" title={comment.text}> {index + 1} </div>
                ))}
            </div>
        </div>
    );
...
function ElementPaletteItem({ config }) {
    ...
    return (
        <div ref={setNodeRef} {...listeners} {...attributes} style={style} className="flex items-center p-2.5 text-left bg-slate-100/50 rounded-lg cursor-grab hover:bg-green-50 transition-all group focus:outline-none focus-visible:ring-1 focus-visible:ring-green-500">
            <div className="w-8 h-8 flex items-center justify-center text-slate-500 group-hover:text-green-600 mr-3 transition-colors">{IconToShow}</div>
...
function AiModeView({ onBack, onAiSubmit, isAiLoading, aiChatHistory }) {
    ...
    return (
        <div className="p-3 flex flex-col h-full">
            <button onClick={onBack} className="flex items-center gap-2 text-sm text-slate-600 hover:text-green-700 mb-4 p-1 rounded-md"><LucideIcons.ArrowLeft className="w-4 h-4" /> Back to Elements</button>
            <div className="relative">
                <textarea onKeyDown={handleKeyDown} placeholder="e.g., create a sleek corporate landing page..." className="w-full p-2 pr-8 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-green-400 focus:border-green-400" rows="3" />
...
function LeftPanel({ isOpen, onClose, onAddTopLevelSection, pages, activePageId, onAddPage, onSelectPage, onRenamePage, onDeletePage, onAiSubmit, isAiLoading, aiChatHistory, onSwitchToAiMode, onSelect, selectedItemId }) {
  ...
  const TabButton = ({ tabName, icon, label }) => (
    <div className="flex-1 relative">
        <button onClick={() => setActiveTab(tabName)} className={`w-full p-3 text-sm font-medium flex items-center justify-center gap-2 transition-colors ${activeTab === tabName ? "text-green-600" : "text-slate-500 hover:text-slate-800"}`}>
            {icon} {label}
        </button>
  ...
  return (
    <aside className={`absolute top-0 left-0 h-full w-80 bg-white/80 border-r border-slate-200/80 backdrop-blur-lg flex-shrink-0 flex flex-col print-hidden transition-transform duration-300 ease-in-out z-40 ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex justify-between items-center p-3 border-b border-slate-200/80 h-[60px]">
        ...
        <div className="flex-1 overflow-y-auto">
            ...
            {activeTab === 'insert' && !isAiMode && (
                <div className="p-3 space-y-4">
                    <div className="relative"><LucideIcons.Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input type="text" placeholder="Search..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-9 pr-3 py-2 bg-white border border-slate-300 rounded-lg text-sm focus:ring-1 focus:ring-green-500" /></div>
                    ...
                    <div>
                        ...
                        <button onClick={onAddTopLevelSection} className="w-full flex items-center p-2.5 text-left bg-slate-100/50 rounded-lg cursor-pointer hover:bg-green-50 transition-all group focus:outline-none focus-visible:ring-1 focus-visible:ring-green-500">
                            <div className="w-8 h-8 flex items-center justify-center text-slate-500 group-hover:text-green-600 mr-3 transition-colors"><LucideIcons.PlusSquare className="w-6 h-6" /></div>
                            <span className="text-sm font-medium text-slate-700 group-hover:text-green-800">Add Section</span>
                        </button>
                    </div>
                    {filteredElements.length > 0 && <div><h3 className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-2 px-1">Elements</h3><div className="space-y-1.5">{filteredElements.map((elConf) => (<ElementPaletteItem key={elConf.id} config={elConf} />))}</div></div>}
                    ...
                    <ul className="space-y-1">
                        {Object.values(pages).map((page) => (
                            <li key={page.id} className="group">
                                <button onClick={() => onSelectPage(page.id)} className={`w-full text-left px-3 py-2 text-sm rounded-lg transition-all duration-150 flex items-center justify-between ${activePageId === page.id ? "bg-green-600 text-white font-semibold shadow-sm" : "text-slate-600 hover:bg-slate-100 hover:text-slate-900"}`}>
...
function RightSidebar({ selectedItemData, onUpdateSelectedProps, pages, activePageId, onRenamePage, onAddGlobalElement, comments, onUpdateComment, onDeleteComment }) {
  ...
  const renderPropertiesPanel = () => {
    if (!selectedItemData || !selectedItemData.itemType) {
        return ( <div className="flex flex-col items-center justify-center h-full text-center p-4"><LucideIcons.MousePointerSquareDashed className="w-16 h-16 text-slate-300 mb-4"/><h3 className="font-semibold text-slate-700">Nothing Selected</h3><p className="text-sm text-slate-500 mt-1">Click an element on the canvas.</p></div> );
    }
...
function CanvasToolbar({ selectedItem, zoom, onZoomChange, onSelect, pages, activeTool, onToolChange }) {
    ...
    return (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-white/70 backdrop-blur-lg h-12 p-1.5 rounded-full border border-slate-200/80 flex items-center justify-between text-sm z-30 shadow-md">
    ...
    return (
        <header className="bg-white/95 backdrop-blur-sm h-[60px] p-2.5 border-b border-slate-200/80 shadow-sm print-hidden flex justify-between items-center px-4 z-[50] relative">
            <div className="flex items-center gap-3">
                ...
                <a href="#" className="flex items-center gap-2.5 text-green-600 font-bold text-lg">
                    <LucideIcons.Feather className="w-6 h-6" />
                    <span className="hidden sm:inline">PageBuilder</span>
                </a>
            </div>
            
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
                <div className="flex items-center justify-center p-1 bg-slate-200/70 rounded-full border border-slate-300/50 gap-1 shadow-inner">
                    <button onClick={() => isPreviewMode && onTogglePreview()} className={`px-4 py-1.5 rounded-full flex items-center gap-2 transition-all duration-300 text-sm font-medium ${!isPreviewMode ? "bg-white text-green-700 shadow" : "text-slate-500 hover:bg-white/60"}`}>
...
export default function ElementBuilderPage({ onExternalSave, initialBuilderState }) {
  ...
  useEffect(() => {
    if (newlyAddedElementId && pages) {
        ...
        for (const pageId in pages) {
            const result = findItemAndPathRecursive(pages[pageId].layout, newlyAddedElementId);
            if (result) { foundItem = result.item; foundPath = `pages[${pageId}].layout${result.path}`; break; }
        }
  ...
  const findContainer = (items, id) => {
    ...
    for (let i = 0; i < items.length; i++) {
        const item = items[i];
        if (item.id === id) return { container: items, index: i + 1 };
        if (item.type === 'column' && `col-${item.id}` === id) { if (!item.elements) item.elements = []; return { container: item.elements, index: item.elements.length }; }
        const childContainer = item.elements ? findContainer(item.elements, id) : null; if (childContainer) return childContainer;
        const columnContainer = item.columns ? findContainer(item.columns, id) : null; if (columnContainer) return columnContainer;
    }
    return null;
  };
  ...
  const handleDragEnd = (e) => {
    ...
    const overId = over.id; const overData = over.data.current;
    ...
    if (activeType === 'canvasElement') {
        setPages(currentPages => {
            ...
            const sourceContainer = findContainer(newPagesState[activePageId]?.layout, active.id);
            if (sourceContainer) {
                const sourceIndex = sourceContainer.index - 1; const [movedElement] = sourceContainer.container.splice(sourceIndex, 1);
                const destContainer = findContainer(newPagesState[activePageId]?.layout, overId);
                if (destContainer) { destContainer.container.splice(destContainer.index, 0, movedElement); }
                else { sourceContainer.container.splice(sourceIndex, 0, movedElement); }
            } return newPagesState;
        });
    }
    if (activeType === 'section') {
        setPages(currentPages => {
            const layout = currentPages[activePageId]?.layout; if(!layout) return currentPages;
            const activeIndex = layout.findIndex(s => s.id === active.id); const overIndex = layout.findIndex(s => s.id === overId);
            if (activeIndex !== -1 && overIndex !== -1) {
                const newPagesState = JSON.parse(JSON.stringify(currentPages));
                newPagesState[activePageId].layout = arrayMove(newPagesState[activePageId].layout, activeIndex, overIndex);
                return newPagesState;
            } return currentPages;
        });
    }
