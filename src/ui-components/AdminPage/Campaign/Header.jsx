...
// --- ELEMENT DISPLAY COMPONENTS (Styling and interaction refined) ---
...
// --- PROPERTY PANEL MANAGEMENT COMPONENTS (Updated) ---
...
function LinkManager({ links, onUpdateLinks, elementId, pages, linkTypeLabel = "Link" }) {
  ...
  // Refined styling for a cleaner look
  return (
...
// --- FULL ELEMENT CONFIG (Unchanged, but icon components updated for consistency) ---
...
// --- MAIN BUILDER COMPONENTS (Updated) ---
...
function ColumnComponent({ parentPath, columnData, columnIndex, onUpdateProps, onDelete, onSelect, selectedItemId, onOpenStructureModal, isInner = false, isPreviewMode, onNavigate, isDraggable, }) {
  ...
  // Refined styling for droppable area and selection
  return (
...
function SectionComponent({ sectionData, sectionIndex, onUpdateProps, onDelete, onSelect, selectedItemId, onOpenStructureModal, isPreviewMode, onNavigate, pageId, isDraggable, }) {
  ...
  // Refined styling for sections and their interactions
...
// --- APP LAYOUT COMPONENTS (Significantly updated) ---
...
function DeviceFrame({ device, page, globalNavbar, globalFooter, onUpdateProps, onDelete, onSelect, selectedItemId, onOpenStructureModal, isPreviewMode, onNavigate, onDeleteGlobalElement, isDraggable, comments, onAddComment, activeTool, }) {
    ...
    // New styled frame and empty state
    return (
...
function StructureSelectorModal({ isOpen, onClose, onSelectStructure, context }) {
    if (!isOpen) return null;
    // New, more visually appealing structure selector
    return (
...
function ElementPaletteItem({ config }) {
    ...
    // Refined palette item styling with group-hover
    return (
...
// New Accordion component for Left Panel organization
...
// ** HEAVILY REFACTORED LeftPanel component **
...
// ** HEAVILY REFACTORED RightSidebar component **
function RightSidebar({ selectedItemData, onUpdateSelectedProps, pages, activePageId, onRenamePage, onAddGlobalElement, comments, onUpdateComment, onDeleteComment }) {
  ...
  // New PropertyGroup component for organization
  ...
  // New custom-styled Toggle Switch
...
function htmlToBuilderJson(htmlString) {
    ...
    function parseNode(node) {
        ...
        // *** CHANGE IS HERE: Detect accordion by its structure, not a specific class ***
        ...
        } else if (node.tagName === 'BUTTON' && !node.classList.contains('accordion-toggle')) { // Make sure not to parse accordion button as a standalone button
        ...
        } else if (isAccordionWrapper) { // *** THIS IS THE NEW LOGIC ***
        ...
        if (element) {
            ...
            // Also check parent for alignment classes if the element itself doesn't have one
    ...
    function parseContainer(containerEl) {
        ...
        containerEl.childNodes.forEach(childNode => {
            ...
            // Attempt to parse the node directly
            const parsedEl = parseNode(childNode);
            if (parsedEl) {
                elements.push(parsedEl);
            } else if (childNode.children.length > 0) {
                // If the node itself isn't a recognized element, parse its children
    ...
    if (contentRoot) {
        contentRoot.querySelectorAll(':scope > section').forEach(sectionEl => {
            ...
            // A simple heuristic to detect a multi-column layout
...
// --- MAIN PAGE COMPONENT (Updated with new layout logic) ---
...
export default function ElementBuilderPage({ onExternalSave, initialBuilderState }) {
  ...
  const [zoom, setZoom] = useState(0.5); // New default zoom
