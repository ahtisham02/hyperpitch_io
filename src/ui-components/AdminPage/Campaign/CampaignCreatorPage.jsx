                ...
                <React.Fragment key={step.id}>
                    <li>
                        ...
                        >
                            ...
                            <span className={`mt-2 text-xs font-semibold max-w-[80px] md:max-w-xs truncate transition-colors duration-200 ${currentStep === step.id ? "text-green-700" : ""} ${currentStep > step.id ? "text-slate-700" : ""} ${currentStep < step.id ? "text-slate-500" : ""}`}>
                                {step.name}
                            </span>
                        </button>
                    </li>
                    {stepIdx < steps.length - 1 && (
                        <div className={`flex-auto border-t-2 transition-colors duration-300 ${currentStep > step.id ? 'border-green-500' : 'border-slate-300'}`} />
                    )}
                </React.Fragment>
            ))}
        </ol>
...
const SummaryCard = ({ title, icon, children }) => (
    <div className="bg-white p-5 rounded-xl shadow-lg border border-slate-200">
        <h3 className="text-md font-semibold text-slate-700 mb-3 flex items-center">{React.cloneElement(icon, { className: "w-5 h-5 mr-2 text-green-600 stroke-[2]" })}{title}</h3>
...
const AICommandInput = ({ onCommand, currentBuilderData, fullTemplates }) => {
    ...
    const addComponentToLayout = (builderData, componentType) => {
        ...
        if (componentType === 'section') {
            const newSection = { id: mockGenerateId("section"), type: "section", props: { paddingTop: "40px", paddingBottom: "40px" }, columns: [{ id: mockGenerateId("col"), type: "column", props: { width: "100%" }, elements: [{ id: mockGenerateId("text"), type: "textBlock", props: { text: "This is a new AI-generated section. Add elements here.", textAlign: "text-center" } }] }] };
        ...
        switch (componentType) {
            case "header": newElement = { id: mockGenerateId("heading"), type: "header", props: { text: "New AI Generated Heading", sizeClass: "text-4xl", textAlign: "text-center", textColor: "#1e293b", fontWeight: "font-bold" } }; break;
            case "paragraph": newElement = { id: mockGenerateId("para"), type: "textBlock", props: { text: "This is a new paragraph added by AI. You can edit this text and its styles in the panel on the right.", sizeClass: "text-base", textAlign: "text-center", textColor: "#334155" } }; break;
            case "icon": newElement = { id: mockGenerateId("icon"), type: "icon", props: { iconName: "Star", size: "48px", color: "#22c55e", textAlign: "text-center" } }; break;
            case "image": newElement = { id: mockGenerateId("img"), type: "image", props: { src: `https://picsum.photos/seed/${Math.random()}/800/400`, alt: "AI generated image", borderRadius: "12px" } }; break;
            case "button": newElement = { id: mockGenerateId("btn"), type: "button", props: { buttonText: "AI Button", link: "#", sizeClass: "text-base", textAlign: "text-center", backgroundColor: "#2e8b57", textColor: "#ffffff", borderRadius: "8px" } }; break;
            case "cardSlider": newElement = { id: mockGenerateId("slider"), type: "cardSlider", props: { slides: [{ id: mockGenerateId("slide"), imgSrc: `https://picsum.photos/seed/${Math.random()}/600/400`, heading: "AI Slide 1", text: "Description for the first slide.", link: "#" }, { id: mockGenerateId("slide"), imgSrc: `https://picsum.photos/seed/${Math.random()}/600/400`, heading: "AI Slide 2", text: "Description for the second slide.", link: "#" }, { id: mockGenerateId("slide"), imgSrc: `https://picsum.photos/seed/${Math.random()}/600/400`, heading: "AI Slide 3", text: "Description for the third slide.", link: "#" }], slidesPerView: 1.5, spaceBetween: 20, cardBorderRadius: "12px" } }; break;
            case "spacer": newElement = { id: mockGenerateId("spacer"), type: "spacer", props: { height: "40px" } }; break;
            case "divider": newElement = { id: mockGenerateId("divider"), type: "divider", props: { color: "#cbd5e1", thickness: "1px", marginY: "20px" } }; break;
            case "map": newElement = { id: mockGenerateId("map"), type: "map", props: { address: "1600 Amphitheatre Parkway, Mountain View, CA", zoom: 14, height: "400px" } }; break;
            default: return builderData;
        }
        
        if (activePage.layout.length === 0) {
            const newSection = { id: mockGenerateId("section"), type: "section", props: { paddingTop: "20px", paddingBottom: "20px" }, columns: [{ id: mockGenerateId("col"), type: "column", props: { width: "100%" }, elements: [newElement] }] };
            activePage.layout.push(newSection);
        } else {
            const lastSection = activePage.layout[activePage.layout.length - 1];
            if (!lastSection.columns || lastSection.columns.length === 0) {
                lastSection.columns = [{ id: mockGenerateId("col"), type: "column", props: { width: "100%" }, elements: [] }];
            }
    ...
    const commandMap = {
        "add a heading": () => addComponentToLayout(currentBuilderData, 'header'), "add heading": () => addComponentToLayout(currentBuilderData, 'header'),
        "add a paragraph": () => addComponentToLayout(currentBuilderData, 'paragraph'), "add text": () => addComponentToLayout(currentBuilderData, 'paragraph'),
        "add an icon": () => addComponentToLayout(currentBuilderData, 'icon'), "add icon": () => addComponentToLayout(currentBuilderData, 'icon'),
        "add a section": () => addComponentToLayout(currentBuilderData, 'section'), "add section": () => addComponentToLayout(currentBuilderData, 'section'), "add an inner section": () => addComponentToLayout(currentBuilderData, 'section'),
        "add an image": () => addComponentToLayout(currentBuilderData, 'image'), "add image": () => addComponentToLayout(currentBuilderData, 'image'),
        "add a button": () => addComponentToLayout(currentBuilderData, 'button'), "add button": () => addComponentToLayout(currentBuilderData, 'button'),
        "add a card slider": () => addComponentToLayout(currentBuilderData, 'cardSlider'), "add a slider": () => addComponentToLayout(currentBuilderData, 'cardSlider'), "add a carousel": () => addComponentToLayout(currentBuilderData, 'cardSlider'),
        "add a spacer": () => addComponentToLayout(currentBuilderData, 'spacer'), "add spacer": () => addComponentToLayout(currentBuilderData, 'spacer'),
        "add a divider": () => addComponentToLayout(currentBuilderData, 'divider'), "add divider": () => addComponentToLayout(currentBuilderData, 'divider'),
        "add a map": () => addComponentToLayout(currentBuilderData, 'map'), "add map": () => addComponentToLayout(currentBuilderData, 'map'),
        "create a sleek corporate landing page": () => fullTemplates.find(t => t.id === "tpl_corporate_sleek")?.builderData,
        "build a vibrant and creative portfolio": () => fullTemplates.find(t => t.id === "tpl_creative_vibrant")?.builderData,
        "create a vibrant and creative portfolio": () => fullTemplates.find(t => t.id === "tpl_creative_vibrant")?.builderData,
        "generate a modern product launch page": () => fullTemplates.find(t => t.id === "tpl_product_launch")?.builderData,
        "make a professional webinar registration page": () => fullTemplates.find(t => t.id === "tpl_webinar_invite")?.builderData,
    };
    ...
    const handleKeyDown = (e) => {
        if (e.key === "Enter" && command && !isLoading) {
            ...
            setTimeout(() => {
                if (action) {
                    const result = action();
                    if (result) { onCommand(result); setCommand(""); } 
                    else { setError("Could not generate the requested content."); }
                } else { setError("Command not recognized. Please try another command."); }
                setIsLoading(false);
            }, 1200);
        }
    };
    ...
    return (
        <div className="mt-4 p-4 border border-slate-200 rounded-xl bg-slate-50/70 animate-fadeIn">
            <div className="relative">
                ...
                <input
                    type="text" value={command} onChange={(e) => setCommand(e.target.value)} onKeyDown={handleKeyDown} disabled={isLoading}
                ...
                />
                {isLoading && <div className="absolute right-3 top-1/2 -translate-y-1/2"><LucideIcons.Loader className="w-5 h-5 text-slate-400 animate-spin" /></div>}
            </div>
...
const initialDataSource = { type: "file", file: null, fileName: "", fields: { firstName: "", lastName: "", email: "", linkedInUrl: "" }, contactListId: null, selectedContactIds: [] };
...
export default function CampaignCreatorPage() {
    ...
    useEffect(() => {
        if (campaignId && campaignId !== currentLoadedCampaignId) {
            ...
            if (campaignToEdit) {
                ...
                setCampaignDetails(campaignToEdit.campaignDetails || initialCampaignDetails);
                setDataSource(campaignToEdit.dataSource || initialDataSource);
                setTemplateConfig(campaignToEdit.templateConfig || initialTemplateConfig);
                setCurrentStep(1);
            } else {
    ...
    const mockTemplates = useMemo(() => {
        ...
        return [
            { id: "tpl_corporate_sleek", name: "Sleek Corporate Testimonial", builderData: { pages: {[pageId1]: {id: pageId1, name: "Main Page", layout: [{ id: mockGenerateId("section"), type: "section", props: { backgroundType: "color", backgroundColor: "#f8fafc", paddingTop: "60px", paddingBottom: "60px" }, columns: [ { id: mockGenerateId("col"), type: "column", props: { width: "100%" }, elements: [{ id: mockGenerateId("heading"), type: "header", props: { text: "What Our Clients Say", sizeClass: "text-4xl", textAlign: "text-center", textColor: "#1e293b", fontWeight: "font-bold" } }, { id: mockGenerateId("spacer"), type: "spacer", props: { height: "40px" } }, { id: mockGenerateId("testimonials"), type: "cardSlider", props: { slides: [ { id: mockGenerateId("slide"), imgSrc: "https://randomuser.me/api/portraits/men/32.jpg", heading: "Alex Johnson, CEO of TechCorp", text: '"Working with this team has been a game changer for our business. Highly recommended!"', link: "#", }, { id: mockGenerateId("slide"), imgSrc: "https://randomuser.me/api/portraits/women/44.jpg", heading: "Maria Garcia, Marketing Director", text: '"Their strategic insights and creative execution are top-notch. Results exceeded expectations."', link: "#", }, { id: mockGenerateId("slide"), imgSrc: "https://randomuser.me/api/portraits/men/75.jpg", heading: "David Lee, Founder of StartUpX", text: '"Professional, responsive, and delivered outstanding quality. Will definitely partner again."', link: "#", },], slidesPerView: 1, spaceBetween: 20, speed: 700, autoplay: true, autoplayDelay: 5000, loop: true, showNavigation: true, showPagination: true, cardBorderRadius: "12px",},},],},],},],},}, activePageId: pageId1, globalNavbar: { id: "global-navbar-corp", type: "navbar", path: "globalNavbar", props: { logoType: "text", logoText: "CORP.", links: [ { id: mockGenerateId("nav"), text: "Services", url: "#" }, { id: mockGenerateId("nav"), text: "About Us", url: "#" }, { id: mockGenerateId("nav"), text: "Contact", url: "#" }, ], backgroundColor: "#ffffff", textColor: "#1e293b", linkColor: "#0ea5e9", rightContentType: "none", }, }, globalFooter: { id: "global-footer-corp", type: "footer", path: "globalFooter", props: { copyrightText: `© ${new Date().getFullYear()} CORP. Solutions. All Rights Reserved.`, links: [ { id: mockGenerateId("foot"), text: "Privacy Policy", url: "#" }, { id: mockGenerateId("foot"), text: "Terms of Use", url: "#" }, ], backgroundColor: "#1e293b", textColor: "#94a3b8", linkColor: "#e0f2fe", },}, }, },
            { id: "tpl_creative_vibrant", name: "Vibrant Creative Showcase", builderData: { pages: {[pageId2]: {id: pageId2, name: "Showcase Page", layout: [{ id: mockGenerateId("section"), type: "section", props: { backgroundType: "color", backgroundColor: "#ecfdf5", paddingTop: "80px", paddingBottom: "80px" }, columns: [{ id: mockGenerateId("col"), type: "column", props: { width: "100%" }, elements: [{ id: mockGenerateId("head"), type: "header", props: { text: "Our Creative Portfolio", sizeClass: "text-5xl", textAlign: "text-center", textColor: "#059669", fontWeight: "font-extrabold" } }, { id: mockGenerateId("sub"), type: "textBlock", props: { text: "We love making our clients happy. Here's a showcase of our best work.", sizeClass: "text-xl", textAlign: "text-center", textColor: "#047857" } },],},],}, { id: mockGenerateId("section-gallery"), type: "section", props: { backgroundType: "color", backgroundColor: "#f0fdfa", paddingTop: "20px", paddingBottom: "80px" }, columns: [{ id: mockGenerateId("col"), type: "column", props: { width: "100%" }, elements: [{ id: mockGenerateId("gallery"), type: "cardSlider", props: { slides: [ { id: mockGenerateId("slide"), imgSrc: "https://images.unsplash.com/photo-1558981403-c5f9899a28bc?q=80&w=2070&auto=format&fit=crop", heading: "Project Alpha", text: "A bold branding project for a new tech startup.", link: "#", }, { id: mockGenerateId("slide"), imgSrc: "https://images.unsplash.com/photo-1558981806-ec527fa84c39?q=80&w=2070&auto=format&fit=crop", heading: "Project Beta", text: "Web design and development for an e-commerce giant.", link: "#", }, { id: mockGenerateId("slide"), imgSrc: "https://images.unsplash.com/photo-1558980394-0a06c4631733?q=80&w=1974&auto=format&fit=crop", heading: "Project Gamma", text: "An immersive mobile app experience.", link: "#", },], slidesPerView: 2.5, spaceBetween: 24, speed: 600, autoplay: false, loop: false, showNavigation: true, showPagination: false, cardBorderRadius: "16px", },},],},],},],},}, activePageId: pageId2, globalNavbar: { id: "global-navbar-creative", type: "navbar", path: "globalNavbar", props: { logoType: "text", logoText: "CreativeHub", links: [ { id: mockGenerateId("nav"), text: "Portfolio", url: "#" }, { id: mockGenerateId("nav"), text: "Blog", url: "#" }, { id: mockGenerateId("nav"), text: "Join Us", url: "#" }, ], backgroundColor: "#0d9488", textColor: "#ffffff", linkColor: "#ccfbf1", rightContentType: "searchIcon", },}, globalFooter: { id: "global-footer-creative", type: "footer", path: "globalFooter", props: { copyrightText: `© ${new Date().getFullYear()} CreativeHub Studios. Be Bold. Be Creative.`, links: [ { id: mockGenerateId("foot"), text: "Inspiration", url: "#" }, { id: mockGenerateId("foot"), text: "Support", url: "#" }, { id: mockGenerateId("foot"), text: "Careers", url: "#" }, ], backgroundColor: "#f0fdfa", textColor: "#0f766e", linkColor: "#14b8a6", },}, }, },
            { id: "tpl_product_launch", name: "Modern Product Launch", builderData: { pages: {[pageId3]: {id: pageId3, name: "Launch Page", layout: [{ id: mockGenerateId("sec-hero"), type: "section", props: { backgroundType: "color", backgroundColor: "#111827", paddingTop: "100px", paddingBottom: "100px" }, columns: [{ id: mockGenerateId("col-hero"), type: "column", props: { width: "100%" }, elements: [{ id: mockGenerateId("head"), type: "header", props: { text: "Introducing The Future of Innovation", sizeClass: "text-6xl", textAlign: "text-center", textColor: "#ffffff", fontWeight: "font-bold" } }, { id: mockGenerateId("sub"), type: "textBlock", props: { text: "Our new product is engineered to perfection, designed for you. Discover a new era of excellence.", sizeClass: "text-xl", textAlign: "text-center", textColor: "#d1d5db" } }, { id: mockGenerateId("spacer"), type: "spacer", props: { height: "20px" } }, { id: mockGenerateId("btn"), type: "button", props: { buttonText: "Pre-Order Now", link: "#", sizeClass: "text-lg", textAlign: "text-center", backgroundColor: "#22c55e", textColor: "#ffffff", borderRadius: "8px", variant: "solid", fullWidth: false } },],},],}, { id: mockGenerateId("sec-features"), type: "section", props: { paddingTop: "80px", paddingBottom: "80px" }, columns: [{ id: mockGenerateId("col-f1"), type: "column", props: { width: "33.33%" }, elements: [{ id: mockGenerateId("icon"), type: "icon", props: { iconName: "Feather", size: "48px", color: "#22c55e" } }, { id: mockGenerateId("head"), type: "header", props: { text: "Lightweight Design", sizeClass: "text-2xl", textAlign: "text-center", fontWeight: "font-semibold" } }, { id: mockGenerateId("text"), type: "textBlock", props: { text: "Crafted with ultra-light materials for maximum portability without compromising on quality.", textAlign: "text-center" } },],},{ id: mockGenerateId("col-f2"), type: "column", props: { width: "33.33%" }, elements: [{ id: mockGenerateId("icon"), type: "icon", props: { iconName: "Zap", size: "48px", color: "#22c55e" } }, { id: mockGenerateId("head"), type: "header", props: { text: "Blazing Fast Speed", sizeClass: "text-2xl", textAlign: "text-center", fontWeight: "font-semibold" } }, { id: mockGenerateId("text"), type: "textBlock", props: { text: "Powered by the latest generation processor for an incredibly responsive user experience.", textAlign: "text-center" } },],},{ id: mockGenerateId("col-f3"), type: "column", props: { width: "33.33%" }, elements: [{ id: mockGenerateId("icon"), type: "icon", props: { iconName: "BatteryCharging", size: "48px", color: "#22c55e" } }, { id: mockGenerateId("head"), type: "header", props: { text: "All-Day Battery", sizeClass: "text-2xl", textAlign: "text-center", fontWeight: "font-semibold" } }, { id: mockGenerateId("text"), type: "textBlock", props: { text: "Stay productive and entertained from morning to night with our extended battery life.", textAlign: "text-center" } },],},],},],},}, activePageId: pageId3, }, },
            { id: "tpl_webinar_invite", name: "Professional Webinar Invite", builderData: { pages: {[pageId4]: {id: pageId4, name: "Webinar Page", layout: [{ id: mockGenerateId("sec-main"), type: "section", props: { backgroundType: "color", backgroundColor: "#f9fafb", paddingTop: "60px", paddingBottom: "60px" }, columns: [{ id: mockGenerateId("col-left"), type: "column", props: { width: "60%" }, elements: [{ id: mockGenerateId("head-cat"), type: "textBlock", props: { text: "EXCLUSIVE WEBINAR", sizeClass: "text-sm", textColor: "#166534", fontWeight: "font-bold" } }, { id: mockGenerateId("head-main"), type: "header", props: { text: "Mastering the Art of Digital Marketing", sizeClass: "text-5xl", fontWeight: "font-extrabold", textColor: "#1f2937" } }, { id: mockGenerateId("date"), type: "textBlock", props: { text: "<strong>Date:</strong> October 26, 2024 at 2:00 PM EST", sizeClass: "text-lg", textColor: "#4b5563" } }, { id: mockGenerateId("btn"), type: "button", props: { buttonText: "Register for Free", link: "#", sizeClass: "text-lg", textAlign: "text-left", backgroundColor: "#2e8b57", textColor: "#ffffff", borderRadius: "8px" } },],},{ id: mockGenerateId("col-right"), type: "column", props: { width: "40%" }, elements: [{ id: mockGenerateId("img-speaker"), type: "image", props: { src: "https://randomuser.me/api/portraits/women/68.jpg", borderRadius: "9999px", boxShadow: "0 10px 15px -3px rgb(0 0 0 / 0.1), 0 4px 6px -4px rgb(0 0 0 / 0.1)" } }, { id: mockGenerateId("head-speaker"), type: "header", props: { text: "Jane Doe", textAlign: "text-center", sizeClass: "text-2xl", fontWeight: "font-bold" } }, { id: mockGenerateId("title-speaker"), type: "textBlock", props: { text: "Chief Marketing Officer, Innovate Inc.", textAlign: "text-center", textColor: "#6b7280" } },],},],}, { id: mockGenerateId("sec-agenda"), type: "section", props: { paddingTop: "40px", paddingBottom: "60px" }, columns: [{ id: mockGenerateId("col-agenda"), type: "column", props: { width: "100%" }, elements: [{ id: mockGenerateId("head-agenda"), type: "header", props: { text: "What You Will Learn", sizeClass: "text-4xl", textAlign: "text-center", fontWeight: "font-bold" } }, { id: mockGenerateId("spacer"), type: "spacer", props: { height: "20px" } }, { id: mockGenerateId("text-agenda"), type: "textBlock", props: { text: "• The latest trends in SEO and content marketing\n• How to build a high-converting sales funnel\n• Social media strategies that actually work\n• Q&A session with our expert panel", sizeClass: "text-lg", textColor: "#374151" } },],},],},],},}, activePageId: pageId4, }, },
        ];
    }, []);
    ...
    const handleDataSourceChange = (e) => {
        ...
        if (name === "fileUpload" && type === "file") setDataSource((prev) => ({ ...prev, file: files[0], fileName: files[0]?.name || "" }));
    ...
    const handleSaveCampaign = () => {
        ...
        const campaignIdToSave = isEditing ? campaignId : `camp-${Date.now()}-${Math.random().toString(36).substring(2, 9)}`;
        let analyticsData;
        if (isEditing) {
    ...
    const selectTemplateToEdit = (templateId) => {
        ...
        if (templateToEdit) {
            setTemplateConfig({ type: "edit", selectedTemplateId: templateId, editingTemplate: templateToEdit.builderData, templateData: templateToEdit.builderData });
        }
    };

    const handleAICommand = (newBuilderData) => {
        setTemplateConfig(prev => ({ ...prev, type: "ai", templateData: newBuilderData, editingTemplate: newBuilderData, selectedTemplateId: null }));
    ...
    const handleTemplateOptionClick = (type) => {
        if (type === 'create') {
            ...
            setTemplateConfig({ type: 'create', selectedTemplateId: null, editingTemplate: emptyState, templateData: emptyState });
        } else if (type === 'select') {
            setTemplateConfig({ type: 'select', selectedTemplateId: null, editingTemplate: null, templateData: null });
        } else if (type === 'ai') {
            const currentData = templateConfig.templateData || getEmptyBuilderState();
            setTemplateConfig({ type: 'ai', selectedTemplateId: null, editingTemplate: currentData, templateData: currentData });
        }
    };
    ...
    const renderStepContent = () => {
        ...
        switch (currentStep) {
            ...
            case 2:
                content = (
                    <>
                        <div className="space-y-3 mb-6">
                            <OptionCard title="Upload File (.xls, .xlsx, .csv)" description="Import contacts directly from a spreadsheet." selected={dataSource.type === "file"} onClick={() => handleDataSourceTypeChange("file")} icon={<LucideIcons.FileUp />} />
                            <OptionCard title="From My Contacts" description="Select from your saved contact lists." selected={dataSource.type === "fromContacts"} onClick={() => handleDataSourceTypeChange("fromContacts")} icon={<UserCheck />} />
                        </div>
                        {dataSource.type === "file" && (
                            <div className="mt-5 p-4 border border-slate-200 rounded-xl bg-slate-50/70 animate-fadeIn">
                                ...
                                <InputField label="Choose Spreadsheet File" type="file" name="fileUpload" onChange={handleDataSourceChange} accept=".xls,.xlsx,.csv" icon={<LucideIcons.UploadCloud />} />
                                ...
                                <p className="text-xs text-slate-600 mb-2 font-medium">Map your spreadsheet columns (required if file used):</p>
                                <div className="grid sm:grid-cols-2 gap-x-4 gap-y-0">
                                    <InputField label="First Name Column Header" name="firstName" value={dataSource.fields.firstName} onChange={handleDataSourceChange} placeholder="e.g., FNAME or first_name" />
                                    <InputField label="Last Name Column Header" name="lastName" value={dataSource.fields.lastName} onChange={handleDataSourceChange} placeholder="e.g., LNAME or last_name" />
                                    <InputField label="Email Column Header" name="email" type="text" value={dataSource.fields.email} onChange={handleDataSourceChange} placeholder="e.g., email_address" required={dataSource.type === "file"} />
                                    <InputField label="LinkedIn URL Column Header" name="linkedInUrl" value={dataSource.fields.linkedInUrl} onChange={handleDataSourceChange} placeholder="e.g., linkedin_profile_url" />
                                </div>
                            </div>
                        )}
                        {dataSource.type === "fromContacts" && (
                            <div className="mt-5 p-4 border border-slate-200 rounded-xl bg-slate-50/70 animate-fadeIn">
                                ...
                                {availableContactLists.length > 0 ? (
                                    <>
                                        <div className="mb-4">
                                            <label htmlFor="contactListSelect" className="block text-xs font-medium text-slate-600 mb-1">Choose a Contact List:</label>
                                            <select id="contactListSelect" name="contactListId" value={dataSource.contactListId || ""} onChange={handleDataSourceChange} className="block w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500/70 focus:border-green-500 sm:text-sm bg-white">
                                                <option value="" disabled>-- Select a List --</option>
                                                {availableContactLists.map((list) => (<option key={list.id} value={list.id}>{list.name} ({list.contacts.length} contacts)</option>))}
                                            </select>
                                        </div>
                                        {dataSource.contactListId && contactsInSelectedList.length > 0 && (
                                            <>
                                                <InputField label="Search Contacts in Selected List" name="searchTermInList" value={searchTermInList} onChange={(e) => setSearchTermInList(e.target.value)} placeholder="Search by name or email..." icon={<LucideIcons.Search size={14} />} className="mb-3" />
                                                <p className="text-xs text-slate-600 mb-2">Select contacts for this campaign ({dataSource.selectedContactIds.length} selected):</p>
                                                <div className="max-h-60 overflow-y-auto border border-slate-300 rounded-lg bg-white p-2 space-y-1.5 custom-scrollbar">
                                                    {filteredContactsForSelection.length > 0 ? (filteredContactsForSelection.map((contact) => (
                                                        <div key={contact.id} className="flex items-center p-2 rounded-md hover:bg-slate-100 transition-colors">
                                                            <input type="checkbox" id={`contact-select-${contact.id}`} name={`contact-select-${contact.id}`} checked={dataSource.selectedContactIds.includes(contact.id)} onChange={handleDataSourceChange} className="h-4 w-4 text-green-600 border-slate-300 rounded focus:ring-green-500 mr-2.5 cursor-pointer accent-[#2e8b57]" />
                                                            <label htmlFor={`contact-select-${contact.id}`} className="flex-1 text-xs text-slate-700 cursor-pointer"><span className="font-medium">{contact.userName || "N/A"}</span>{" "}-{" "}<span className="text-slate-500">{contact.email}</span></label>
                                                        </div>
                                                    ))) : (<p className="text-xs text-slate-500 text-center py-3">{searchTermInList ? "No contacts match your search." : "No contacts in this list (or clear search)."}</p>)}
                                                </div>
                                            </>
                                        )}
                                        {dataSource.contactListId && contactsInSelectedList.length === 0 && (<p className="text-xs text-slate-500 text-center py-3">This list is empty.</p>)}
                                    </>
                                ) : (
                                    <div className="text-center py-6">
                                        ...
                                        <p className="text-xs text-slate-400">Please create a contact list in the <Link to="/contacts" className="text-green-600 hover:underline font-medium">'Contacts'</Link> section first.</p>
                                    </div>
                                )}
                            </div>
                        )}
                ...
                break;
            case 3:
                const showEditor = templateConfig.type === 'create' || templateConfig.type === 'edit' || templateConfig.type === 'ai';
                content = (
                    <>
                        <div className="space-y-3 mb-5">
                            <div className="grid md:grid-cols-2 gap-3">
                                <OptionCard title="Create New Template" description="Build a unique template from scratch." selected={templateConfig.type === "create"} onClick={() => handleTemplateOptionClick('create')} icon={<LucideIcons.Brush />} />
                                <OptionCard title="Select Existing Template" description="Choose and customize a pre-built design." selected={templateConfig.type === "select" || (templateConfig.type === "edit" && !!templateConfig.selectedTemplateId)} onClick={() => handleTemplateOptionClick('select')} icon={<LucideIcons.LayoutGrid />} />
                            </div>
                                ...
                                {mockTemplates.length > 0 ? (
                                    <ul className="space-y-2.5">
                                        {mockTemplates.map((template) => (
                                            <li key={template.id} className="p-2.5 pr-3 border border-slate-300/80 rounded-lg hover:shadow-sm transition-shadow flex justify-between items-center bg-white hover:bg-slate-50">
                                                <span className="text-xs font-medium text-slate-600">{template.name}</span>
                                                <StyledButton onClick={() => selectTemplateToEdit(template.id)} variant="secondary" className="py-1 px-2.5 text-xs" iconLeft={<LucideIcons.Edit3 size={12} />}>Edit</StyledButton>
                                            </li>
                                        ))}
                                    </ul>
                        ...
                        {showEditor && (
                            <div className="mt-5">
                                <h3 className="text-lg font-semibold text-slate-800 mb-0.5">
                                    {templateConfig.type === 'edit' ? `Editing: ${mockTemplates.find((t) => t.id === templateConfig.selectedTemplateId)?.name}` : templateConfig.type === 'create' ? 'Create New Template' : 'AI-Assisted Editor'}
                                </h3>
                                ...
                                {ElementBuilderPage ? (
                                    <div className="border-2 border-slate-300 rounded-xl overflow-hidden shadow-lg bg-slate-100" style={{ height: "calc(100vh - 420px)", minHeight: "610px" }}>
                                        <ElementBuilderPage onExternalSave={handleTemplateDataFromBuilder} initialBuilderState={templateConfig.templateData} key={JSON.stringify(templateConfig.templateData)} />
                                    </div>
                                ) : (
                                    <div className="p-4 bg-red-100 border border-red-300 rounded-lg text-red-700">Error: ElementBuilderPage component not loaded. Check console.</div>
                                )}
                            </div>
                        )}
                    </>
                ...
                break;
            case 4:
                const templateName = templateConfig.selectedTemplateId ? mockTemplates.find((t) => t.id === templateConfig.selectedTemplateId)?.name : templateConfig.templateData ? "Custom or AI-Generated Template" : "No Template Selected";
                const currentTemplateData = templateConfig.templateData;
                const selectedContactListName = dataSource.contactListId && availableContactLists.length > 0 ? availableContactLists.find((l) => l.id === dataSource.contactListId)?.name || "N/A" : "N/A";
                content = (
                    <>
                        <div className="grid lg:grid-cols-3 gap-5">
                            <div className="lg:col-span-1 space-y-4">
                                <SummaryCard title="Campaign Settings" icon={<LucideIcons.Settings2 />}><p><strong>Name:</strong> {campaignDetails.campaignName || "N/A"}</p><p><strong>Start:</strong> {campaignDetails.startTime ? new Date(campaignDetails.startTime).toLocaleString() : "N/A"}</p><p><strong>End:</strong> {campaignDetails.endTime ? new Date(campaignDetails.endTime).toLocaleString() : "N/A"}</p></SummaryCard>
                                <SummaryCard title="Audience Data" icon={<AudienceIcon />}><p><strong>Source:</strong>{dataSource.type === "file" && " File Upload"}{dataSource.type === "fromContacts" && " From My Contacts"}</p>{dataSource.type === "file" && dataSource.fileName && (<><p><strong>File:</strong> {dataSource.fileName || "N/A"}</p><p className="mt-1 pt-1 border-t border-slate-200"><strong>Column Mappings:</strong></p><ul className="list-disc list-inside pl-1.5 space-y-0.5">{dataSource.fields.firstName && (<li>First Name: <span className="font-mono bg-slate-100 px-1 py-0.5 rounded text-green-700 text-[11px]">{dataSource.fields.firstName}</span></li>)}{dataSource.fields.lastName && (<li>Last Name: <span className="font-mono bg-slate-100 px-1 py-0.5 rounded text-green-700 text-[11px]">{dataSource.fields.lastName}</span></li>)}<li>Email: <span className="font-mono bg-slate-100 px-1 py-0.5 rounded text-green-700 text-[11px]">{dataSource.fields.email || "N/A"}</span></li>{dataSource.fields.linkedInUrl && (<li>LinkedIn: <span className="font-mono bg-slate-100 px-1 py-0.5 rounded text-green-700 text-[11px]">{dataSource.fields.linkedInUrl}</span></li>)}</ul></>)}{dataSource.type === "fromContacts" && (<><p><strong>List Name:</strong> {selectedContactListName}</p><p><strong>Selected Contacts:</strong> {dataSource.selectedContactIds.length} contacts</p></>)}</SummaryCard>
                            </div>
                            <div className="lg:col-span-2">
                                <SummaryCard title="Template Preview" icon={<LucideIcons.MonitorPlay />}><p className="mb-2 text-xs"><strong>Using:</strong> {templateName}</p><div className="mb-2.5 flex justify-center space-x-1 p-0.5 bg-slate-100 rounded-lg">{["desktop", "tablet", "mobile"].map((device) => (<button key={device} onClick={() => setPreviewDevice(device)} title={`${device.charAt(0).toUpperCase() + device.slice(1)} View`} className={`p-1.5 rounded-md transition-all ${previewDevice === device ? "bg-[#2e8b57] text-white shadow-sm" : "text-slate-500 hover:bg-slate-200 hover:text-slate-700"}`}>{device === "desktop" && <LucideIcons.Monitor className="w-3.5 h-3.5" />}{device === "tablet" && <LucideIcons.Tablet className="w-3.5 h-3.5" />}{device === "mobile" && <LucideIcons.Smartphone className="w-3.5 h-3.5" />}</button>))}</div>{currentTemplateData && currentTemplateData.pages && currentTemplateData.activePageId && PagePreviewRenderer ? (<div className="border-2 border-slate-200 rounded-lg overflow-hidden shadow-lg bg-white" style={{ minHeight: "400px" }}><PagePreviewRenderer pageLayout={currentTemplateData.pages[currentTemplateData.activePageId]?.layout || []} globalNavbar={currentTemplateData.globalNavbar} globalFooter={currentTemplateData.globalFooter} activePageId={currentTemplateData.activePageId} previewDevice={previewDevice} onNavigate={() => {}} /></div>) : (<div className="p-4 bg-slate-100/70 rounded-lg text-slate-500 text-center min-h-[150px] flex flex-col items-center justify-center border border-slate-200"><LucideIcons.ImageOff className="w-6 h-6 mb-1.5 text-slate-400" /><p className="font-medium text-xs">No template preview available.{(!ElementBuilderPage || !PagePreviewRenderer) && "Preview components not loaded."}</p><p className="text-[11px]">Please complete the template design step.</p></div>)}</SummaryCard>
                            </div>
                        </div>
                    </>
                );
                break;
            default: content = (<div className="text-center py-8 text-slate-500">Error: Unknown step. Please restart.</div>);
        }
        return (
            <>
                <div className="bg-white p-5 md:p-6 rounded-xl shadow-lg border border-slate-200">
                    ...
                    <div className="mt-8 pt-5 border-t border-slate-200/80 flex justify-between items-center">
                        ...
                        {currentStep < steps.length ? (
                            <StyledButton onClick={nextStep} disabled={!canProceedToNext()} iconRight={<LucideIcons.ArrowRight />}>{canProceedToNext() ? "Next" : "Complete Current Step"}</StyledButton>
                        ) : (
                            <StyledButton onClick={handleSaveCampaign} variant="launch" iconLeft={<LucideIcons.Save />}>{isEditing ? "Update Campaign" : "Save Campaign"}</StyledButton>
                        )}
                    </div>
                </div>
            </>
                ...
                <main className="w-full max-w-7xl mx-auto p-4 md:p-6 lg:p-8">
                    <div className="flex items-center mb-6 md:mb-8">
                        <div className="p-3 bg-green-100 rounded-xl mr-4">
                            <LucideIcons.ClipboardCheck className="w-8 h-8 text-green-600" strokeWidth={1.5} />
                        </div>
                        <div>
