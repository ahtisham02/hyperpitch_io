...
export default function CampaignCreatorPage() {
    ...
    if (savedData && !campaignId) {
      try {
        ...
        if (
          ...
          Object.keys(parsedData.campaignDetails).length > 0
        )
          setCampaignDetails(parsedData.campaignDetails);
        if (
          parsedData.dataSource &&
          Object.keys(parsedData.dataSource).length > 0
        )
          setDataSource(parsedData.dataSource);
        if (
          parsedData.templateConfig &&
          Object.keys(parsedData.templateConfig).length > 0
        )
  ...
  const getEmptyBuilderState = () => {
    ...
    return {
      pages: { [pageId]: { id: pageId, name: "Untitled", layout: [] } },
      activePageId: pageId,
      globalNavbar: null,
      globalFooter: null,
      pageTitle: "Untitled",
      ...
      if (campaignId && token) {
        ...
        try {
          ...
          setCampaignDetails({
            ...
            startTime: campaignToEdit.startTime
              ? new Date(campaignToEdit.startTime).toISOString().slice(0, 16)
              : "",
            endTime: campaignToEdit.endTime
              ? new Date(campaignToEdit.endTime).toISOString().slice(0, 16)
              : "",
          });
          setDataSource({
          ...
          setTemplateConfig({
            type: "edit",
            selectedTemplateId: null,
            templateData: campaignToEdit.script
              ? JSON.parse(campaignToEdit.script)
              : getEmptyBuilderState(),
            editingTemplate: campaignToEdit.script
              ? JSON.parse(campaignToEdit.script)
              : getEmptyBuilderState(),
          });
        } catch (error) {
  ...
  useEffect(() => {
    if (dataSource.type === "fromContacts") {
      ...
      const contactImports = JSON.parse(
        localStorage.getItem("contactImports") || "[]"
      );
  ...
  const handleDataSourceChange = (e) => {
    ...
    if (name === "fileUpload" && type === "file") {
      ...
      setFileError("");
      setDataSource((prev) => ({ ...prev, file: file, fileName: file.name }));
  ...
  const handleSaveAndCloseFromBuilder = () => {
    ...
    switch (currentStep) {
        ...
        content = (
          <>
            ...
            {dataSource.type === "fromContacts" && (
              <div className="mt-5 p-4 border border-slate-200 rounded-xl bg-slate-50/70 animate-fadeIn relative z-40">
                <h3 className="text-base font-semibold text-slate-700 mb-3">
                      ...
                      <div className="relative z-50">
                        ...
                        {showContactDropdown && (
                          <>
                            ...
                            />
                            <div 
                              className="absolute z-[9999999] w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-xl max-h-60 overflow-y-auto transform translate-y-0" 
                        ...
                        <>
                          ...
                          <div className="max-h-60 overflow-y-auto border border-slate-300 rounded-lg bg-white p-2 space-y-1.5 custom-scrollbar">
                            {filteredContactsForSelection.length > 0 ? (
                              filteredContactsForSelection.map(
                                (contact, index) => {
                                  // Create a stable, unique identifier for each contact
                                  const contactIdentifier = contact.id || `contact-${contact.email}-${index}`;
                                  const isSelected = dataSource.selectedContactIds.includes(contactIdentifier);
                                    ...
                                    >
                                      <input
                                        ...
                                        onChange={(e) => {
                                          ...
                                          console.log('Checkbox change:', { contactIdentifier, newChecked, currentIds: dataSource.selectedContactIds });
                                          
                                          setDataSource((prev) => {
                                            if (newChecked) {
                                              // Add to selection if not already present
                                              if (!prev.selectedContactIds.includes(contactIdentifier)) {
                                                const newIds = [...prev.selectedContactIds, contactIdentifier];
                                                console.log('Adding contact:', { contactIdentifier, newIds });
                                                return {
                                                  ...prev,
                                              ...
                                              const newIds = prev.selectedContactIds.filter(
                                                (id) => id !== contactIdentifier
                                              );
                                              console.log('Removing contact:', { contactIdentifier, newIds });
                                              return {
                                                ...prev,
                                                selectedContactIds: newIds,
                                              };
                                      ...
                                      />
                                      <label
                                        htmlFor={`contact-select-${contactIdentifier}`}
      ...
      case 3:
        const isReturningFromEditor =
          location.state?.fromEditor && location.state?.templateData;
        const showCreatedTemplate =
          (templateConfig.type === "create" &&
            templateConfig.templateData &&
            templateConfig.templateData.pages) ||
          isReturningFromEditor;
        content = (
          <>
            <div className="mb-5">
              ...
              <p className="text-sm text-slate-600 mb-4">
                Choose how you want to design your campaign template
              </p>
            ...
            {showCreatedTemplate && (
              <div className="mt-5">
                <h3 className="text-lg font-semibold text-slate-800 mb-0.5">
                  Using:{" "}
                  {templateConfig.templateData?.pageTitle ||
                    location.state?.templateData?.pageTitle ||
                    "Custom Template"}
                </h3>
                ...
                <div className="p-4 bg-green-50 border border-green-200 rounded-lg">
                  ...
                  <div className="mt-2 text-xs text-slate-600">
                    <p>
                      <strong>Template Type:</strong> Custom Landing Page
                    </p>
                    <p>
        ...
        break;
      case 4:
        const savedTemplateData = localStorage.getItem(
          "campaign_template_data"
        );
        ...
        if (!currentTemplateData) {
          currentTemplateData =
            templateConfig.templateData || location.state?.templateData;
        }
        const templateName = templateConfig.selectedTemplateId
          ? templates.find((t) => t.id === templateConfig.selectedTemplateId)
              ?.name
          : currentTemplateData
      ...
      <div className="bg-white p-5 md:p-6 rounded-xl shadow-lg border border-slate-200">
        ...
        <div className="mt-8 pt-5 border-t border-slate-200/80 flex justify-between items-center">
          ...
          {currentStep < steps.length ? (
            ...
            >
              {canProceedToNext() ? "Next" : "Complete Step"}
            </StyledButton>
          ) : (
            <StyledButton
      ...
      {showTemplateEditor && (
        <div className="fixed inset-0 bg-white z-50 flex flex-col">
          <div className="flex-1 overflow-hidden">
            {ElementBuilderPage ? (
              <ElementBuilderPage
                onSaveAndClose={handleSaveAndCloseFromBuilder}
                onCancel={handleCancelFromBuilder}
                onExternalSave={handleTemplateDataFromBuilder}
                onDataChange={handleTemplateDataFromBuilder}
                initialBuilderState={
                  templateConfig.editingTemplate || getEmptyBuilderState()
                }
              />
            ) : (
