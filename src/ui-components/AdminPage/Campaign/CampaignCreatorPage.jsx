...
export default function CampaignCreatorPage() {
    ...
    useEffect(() => {
        if (campaignDetails.campaignName || dataSource.type || templateConfig.type) {
            const campaignData = { campaignDetails, dataSource, templateConfig, currentStep };
    ...
    useEffect(() => {
        ...
        if (savedData && !campaignId) {
            try {
                const parsedData = JSON.parse(savedData);
                if (parsedData.campaignDetails && Object.keys(parsedData.campaignDetails).length > 0) setCampaignDetails(parsedData.campaignDetails);
                if (parsedData.dataSource && Object.keys(parsedData.dataSource).length > 0) setDataSource(parsedData.dataSource);
                if (parsedData.templateConfig && Object.keys(parsedData.templateConfig).length > 0) setTemplateConfig(parsedData.templateConfig);
                if (parsedData.currentStep && parsedData.currentStep >= 1 && parsedData.currentStep <= 4) setCurrentStep(parsedData.currentStep);
            } catch (error) {
    ...
    useEffect(() => {
        if (location.state?.fromEditor && location.state?.templateData && !templateConfig.templateData) {
            const newTemplateConfig = { type: "create", templateData: location.state.templateData, editingTemplate: location.state.templateData };
        ...
        if (savedTemplateData && !templateConfig.templateData) {
            try {
                ...
                if (parsedTemplateData.pages && parsedTemplateData.activePageId) {
                    setTemplateConfig(prev => ({ ...prev, type: "create", templateData: parsedTemplateData, editingTemplate: parsedTemplateData }));
                }
            } catch (error) {
    ...
    const getEmptyBuilderState = () => {
        ...
        return { pages: { [pageId]: { id: pageId, name: "Untitled", layout: [] } }, activePageId: pageId, globalNavbar: null, globalFooter: null, pageTitle: "Untitled" };
    };
    ...
    useEffect(() => {
        if (location.state?.fromEditor && location.state?.templateData) {
            ...
            setTemplateConfig(newTemplateConfig);
            setTimeout(() => {
                setTemplateConfig(prev => ({ ...prev, type: "create", templateData: location.state.templateData, editingTemplate: location.state.templateData }));
            }, 100);
            setCurrentStep(3);
    ...
    useEffect(() => {
        const createSampleContacts = () => {
            ...
            if (!existingContacts) {
                const sampleContacts = [{ id: 'sample-list-1', listName: 'Sample Contacts', name: 'Sample Contacts', contacts: [{ id: 'contact-1', userName: 'John Smith', email: 'john.smith@example.com' }, { id: 'contact-2', userName: 'Sarah Johnson', email: 'sarah.johnson@example.com' }, { id: 'contact-3', userName: 'Mike Davis', email: 'mike.davis@example.com' }], createdAt: new Date().toISOString(), contactCount: 3 }];
    ...
    const resetCampaignStates = useCallback(() => {
        ...
        const loadCampaignForEdit = async () => {
            if (campaignId && token) {
                ...
                try {
                    ...
                    setCampaignDetails({ campaignName: campaignToEdit.campaignName || "", startTime: campaignToEdit.startTime ? new Date(campaignToEdit.startTime).toISOString().slice(0, 16) : "", endTime: campaignToEdit.endTime ? new Date(campaignToEdit.endTime).toISOString().slice(0, 16) : "" });
                    setDataSource({ type: campaignToEdit.audienceFile ? 'file' : 'fromContacts', file: null, fileName: campaignToEdit.audienceFile || "", contactListId: campaignToEdit.contactListId || null, selectedContactIds: [] });
                    setTemplateConfig({ type: "edit", selectedTemplateId: null, templateData: campaignToEdit.script ? JSON.parse(campaignToEdit.script) : getEmptyBuilderState(), editingTemplate: campaignToEdit.script ? JSON.parse(campaignToEdit.script) : getEmptyBuilderState() });
                } catch (error) {
    ...
    useEffect(() => {
        if (dataSource.type === "fromContacts") {
            ...
            const convertedImports = contactImports.map(importData => ({ id: importData.id, name: importData.listName || importData.name, contacts: importData.contacts || [] }));
                ...
                setDataSource((prev) => {
                    if (checked) {
                        if (!prev.selectedContactIds.includes(contactId)) {
                            return { ...prev, selectedContactIds: [...prev.selectedContactIds, contactId] };
                        }
                    } else {
                        return { ...prev, selectedContactIds: prev.selectedContactIds.filter((id) => id !== contactId) };
                    }
                    return prev;
                });
            } else {
    ...
    }, [currentStep, campaignDetails, dataSource, templateConfig, location.state]);
    ...
    if (isLoading) {
        return <div className="min-h-screen bg-slate-100 flex items-center justify-center"><LucideIcons.Loader2 className="animate-spin text-green-600 w-12 h-12" /></div>;
    }
        ...
        switch (currentStep) {
                ...
                content = (
                    <>
                        <div className="space-y-3 mb-6">
                            <OptionCard title="Upload File (.xls, .xlsx)" description="Import contacts directly from a spreadsheet." selected={dataSource.type === "file"} onClick={() => handleDataSourceTypeChange("file")} icon={<LucideIcons.FileUp />} />
                        ...
                        {dataSource.type === "file" && (
                            <div className="mt-5 p-4 border border-slate-200 rounded-xl bg-slate-50/70 animate-fadeIn">
                                ...
                                {dataSource.fileName && !fileError && (<p className="text-xs text-slate-500 mb-4 -mt-3">Selected: <span className="font-medium text-green-600">{dataSource.fileName}</span></p>)}
                            </div>
                        )}
                                    ...
                                    <>
                                        <div className="mb-4">
                                            ...
                                            <div className="relative">
                                                <button type="button" onClick={() => setShowContactDropdown(!showContactDropdown)} className="w-full px-3 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500/70 focus:border-green-500 sm:text-sm bg-white text-left flex items-center justify-between">
                                                    <span className={dataSource.contactListId ? "text-slate-800" : "text-slate-500"}>
                                                        {dataSource.contactListId ? availableContactLists.find(l => l.id === dataSource.contactListId)?.name || "Select a List" : "-- Select a List --"}
                                                    </span>
                                                    ...
                                                    <div className="absolute z-50 w-full mt-1 bg-white border border-slate-200 rounded-lg shadow-lg max-h-60 overflow-y-auto">
                                                        <ul className="py-1">
                                                            {availableContactLists.map((list) => (
                                                                <li key={list.id}>
                                                                    <button type="button" onClick={() => { handleDataSourceChange({ target: { name: "contactListId", value: list.id } }); setShowContactDropdown(false); }} className={`w-full px-3 py-2 text-left hover:bg-slate-50 transition-colors ${dataSource.contactListId === list.id ? "bg-green-50 text-green-700 border-r-2 border-green-500" : "text-slate-700"}`}>
                                                                        <div className="flex items-center justify-between">
                                                                            <span className="font-medium">{list.name}</span>
                                                                            <span className="text-xs text-slate-500 bg-slate-100 px-2 py-1 rounded-full">{list.contacts.length} contacts</span>
                                                                        </div>
                                                                    </button>
                                                                </li>
                                                            ))}
                                                        ...
                                                        filteredContactsForSelection.map((contact, index) => {
                                                            ...
                                                            return (
                                                                <div key={contactIdentifier} className="flex items-center p-2 rounded-md hover:bg-slate-100 transition-colors">
                                                                    <input type="checkbox" id={`contact-select-${contactIdentifier}`} name={`contact-select-${contactIdentifier}`} checked={dataSource.selectedContactIds.includes(contactIdentifier)} onChange={(e) => handleDataSourceChange({ target: { name: `contact-select-${contactIdentifier}`, checked: e.target.checked } })} className="h-4 w-4 text-green-600 border-slate-300 rounded focus:ring-green-500 mr-2.5 cursor-pointer accent-[#2e8b57]" />
            ...
            case 3:
                ...
                content = (
                    <>
                        <div className="mb-5">
                            ...
                            <div className="space-y-3">
                                <div className="grid md:grid-cols-2 gap-3">
                                    <OptionCard title="Create New Template" description="Build a unique template from scratch in full-screen editor." selected={templateConfig.type === "create"} onClick={() => handleTemplateOptionClick("create")} icon={<LucideIcons.Brush />} />
                        ...
                        {showCreatedTemplate && (
                            <div className="mt-5">
                                <h3 className="text-lg font-semibold text-slate-800 mb-0.5">Using: {(templateConfig.templateData?.pageTitle || location.state?.templateData?.pageTitle) || "Custom Template"}</h3>
                                    ...
                                    <ul className="space-y-2.5">
                                        {templates.map((template) => (
                                            <li key={template.id} className="p-2.5 pr-3 border border-slate-300/80 rounded-lg hover:shadow-sm transition-shadow flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2 sm:gap-0 bg-white hover:bg-slate-50">
                                                ...
                                                <StyledButton onClick={() => selectTemplateToEdit(template.id)} variant="secondary" className="py-1 px-2.5 text-xs self-start sm:self-center" iconLeft={<LucideIcons.Edit3 size={12} />}>Use Template</StyledButton>
                                            </li>
                                        ))}
                                    </ul>
                                ) : (
            ...
            case 4:
                ...
                if (savedTemplateData) {
                    try { currentTemplateData = JSON.parse(savedTemplateData); } catch (error) { console.error('Error parsing saved template data:', error); }
                }
                if (!currentTemplateData) {
                    currentTemplateData = templateConfig.templateData || location.state?.templateData;
                }
                const templateName = templateConfig.selectedTemplateId ? templates.find((t) => t.id === templateConfig.selectedTemplateId)?.name : currentTemplateData ? (currentTemplateData.pageTitle || "Custom Template") : "No Template";
            ...
            {showTemplateEditor && (
                <div className="fixed inset-0 bg-slate-900/95 backdrop-blur-sm z-50 flex flex-col">
                    <div className="flex items-center justify-between p-4 bg-white border-b border-slate-200 shadow-sm">
                        ...
                        <div className="flex items-center space-x-3">
                            <StyledButton onClick={() => setShowTemplateEditor(false)} variant="secondary" iconLeft={<LucideIcons.X />}>Cancel</StyledButton>
                            <StyledButton onClick={() => { if (templateConfig.templateData) { localStorage.setItem('campaign_template_data', JSON.stringify(templateConfig.templateData)); toast.success('Template saved successfully!'); } setShowTemplateEditor(false); if (canProceedToNext()) { setCurrentStep(3); } }} variant="primary" iconLeft={<LucideIcons.Save />}>Save & Close Editor</StyledButton>
                        </div>
                    </div>
                    <div className="flex-1 overflow-hidden">
                        {ElementBuilderPage ? (
                            <ElementBuilderPage 
                                ...
                                onDataChange={handleTemplateDataFromBuilder}
                                initialBuilderState={templateConfig.editingTemplate || getEmptyBuilderState()} 
                            />
                        ) : (
