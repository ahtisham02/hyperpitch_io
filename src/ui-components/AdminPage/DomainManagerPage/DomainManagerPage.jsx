                ...
                <React.Fragment key={step.name}>
                    <li className="relative flex flex-col items-center flex-1">
                        <div className="flex flex-col items-center text-center">
                            ...
                            <span className={`mt-2 text-xs font-semibold max-w-[80px] sm:max-w-[100px] transition-colors duration-200 ${currentStep === step.id ? "text-green-700" : "text-slate-600"}`}>
                                {step.name}
                            </span>
                        </div>
                    </li>
...
const CopyButton = ({ textToCopy }) => {
    ...
    return (
        <button onClick={handleCopy} className="text-xs bg-slate-200 hover:bg-slate-300 text-slate-600 font-semibold py-1 px-3 rounded-md transition-all flex-shrink-0">
...
const OptionCard = ({ title, description, selected, onClick, icon }) => (
    <button type="button" onClick={onClick} className={`w-full p-4 sm:p-6 border-2 rounded-xl text-left transition-all duration-200 group ${selected ? "border-green-500 bg-green-50/80 ring-2 ring-green-200" : "border-slate-300 bg-white hover:border-green-400/70 hover:bg-green-50/30"}`}>
        <div className="flex items-center">
            <div className={`mr-4 p-2 sm:p-3 rounded-lg transition-colors duration-200 ${selected ? "bg-green-100 text-green-600" : "bg-slate-100 text-slate-500 group-hover:bg-green-100"}`}>
...
export default function DomainManagerPage() {
    ...
    const handleVerifyAndConnect = () => {
        ...
        setTimeout(() => {
            const newDomain = {
                id: `dom-${Date.now()}`, name: fullDomain, isPrimary: domains.length === 0, status: 'Connected', type: 'Website pages',
            };
            addDomain(newDomain);
    ...
    const resetWizard = () => {
        setCurrentStep(1); setConnectType(null); setDomainInput(''); setSubdomainInput(''); setVerificationStatus('pending');
    }

    const connectAnother = () => resetWizard();
    const finishSetup = () => { setIsConnecting(false); resetWizard(); }

    const steps = [
        { id: 1, name: 'Choose Type' },
        { id: 2, name: 'Update DNS' },
        { id: 3, name: 'Connected' }
    ];

    const inputBaseStyles = "block w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500/70 focus:border-green-500";
    const labelBaseStyles = "block text-sm font-medium text-slate-700 mb-2";
    ...
    const wizardContent = () => {
        switch (currentStep) {
            case 1:
                return (
                    <div className="w-full max-w-4xl mx-auto text-center">
                        <h2 className="text-2xl md:text-3xl font-bold text-slate-800 mb-2">Let's connect your domain</h2>
                        <p className="text-slate-500 mb-8">First, choose your domain type.</p>
                        
                        <div className="grid md:grid-cols-2 gap-4 md:gap-6">
                            <OptionCard title="Root Domain" description="e.g., yourcompany.com" icon={<LucideIcons.Globe2 />} selected={connectType === 'root'} onClick={() => setConnectType('root')} />
                            <OptionCard title="Subdomain" description="e.g., blog.yourcompany.com" icon={<LucideIcons.Globe />} selected={connectType === 'subdomain'} onClick={() => setConnectType('subdomain')} />
                        </div>
                        ...
                        {connectType && (
                            <div className="mt-8 bg-white p-6 rounded-xl shadow-md border border-slate-200 text-left animate-fade-in">
                                {connectType === 'root' && (
                                    <div>
                                        <label htmlFor="domain" className={labelBaseStyles}>Enter your root domain</label>
                                        <input type="text" id="domain" value={domainInput} onChange={(e) => setDomainInput(e.target.value.toLowerCase())} placeholder="yourdomain.com" className={inputBaseStyles} required />
                                        <p className="text-xs text-slate-500 mt-2">We will set it up as <span className="font-semibold">www.yourdomain.com</span>.</p>
                                    </div>
                                )}
                                {connectType === 'subdomain' && (
                                    <div className="flex flex-col sm:flex-row sm:items-end sm:gap-2">
                                        <div className="flex-shrink-0">
                                            <label htmlFor="subdomain" className={labelBaseStyles}>Subdomain</label>
                                            <input type="text" id="subdomain" value={subdomainInput} onChange={(e) => setSubdomainInput(e.target.value.toLowerCase())} placeholder="blog" className={inputBaseStyles} required />
                                        </div>
                                        <span className="hidden sm:block text-slate-400 text-2xl mx-1 pb-[9px]">.</span>
                                        <div className="flex-grow mt-4 sm:mt-0">
                                            <label htmlFor="domain" className={labelBaseStyles}>Root Domain</label>
                                            <input type="text" id="domain" value={domainInput} onChange={(e) => setDomainInput(e.target.value.toLowerCase())} placeholder="yourdomain.com" className={inputBaseStyles} required />
                                        </div>
                                    </div>
                                )}
                ...
                return (
                    <div className="w-full max-w-5xl mx-auto">
                        <div className="text-center">
                             <h2 className="text-2xl md:text-3xl font-bold text-slate-800 mb-3">Update your DNS records</h2>
                             <p className="text-slate-500 mb-8 max-w-3xl mx-auto">Log in to your domain provider (e.g., GoDaddy, Namecheap) and add the following CNAME record.</p>
                        </div>
                        ...
                        <div className="grid lg:grid-cols-3 gap-8">
                            <div className="lg:col-span-2 bg-white p-6 rounded-xl shadow-md border border-slate-200">
                                ...
                                <div className="space-y-4 lg:hidden">
                                    <div className="font-mono text-sm bg-slate-50 p-3 rounded-md">
                                        <div className="text-xs text-slate-500 font-sans uppercase font-semibold">Type</div>
                                        <div className="text-slate-800">CNAME</div>
                                    </div>
                                    <div className="font-mono text-sm bg-slate-50 p-3 rounded-md">
                                        <div className="text-xs text-slate-500 font-sans uppercase font-semibold">Host / Name</div>
                                        <div className="text-slate-800">{hostName}</div>
                                    </div>
                                    <div className="font-mono text-sm bg-slate-50 p-3 rounded-md">
                                        <div className="text-xs text-slate-500 font-sans uppercase font-semibold">Value / Points to</div>
                                        <div className="flex items-center justify-between gap-4">
                                            <span className="text-slate-800 break-all">{dnsValue}</span>
                                            <CopyButton textToCopy={dnsValue} />
                                        </div>
                                    </div>
                                </div>
                                ...
                                {verificationStatus === 'verifying' && 
                                    <div className="mt-4 text-sm text-blue-700 flex items-center animate-fade-in"><LucideIcons.Loader className="w-4 h-4 mr-2 animate-spin" /> We're checking your records...</div>
                                }
                            </div>
                            <div className="lg:col-span-1 bg-green-50/50 p-6 rounded-xl border border-green-200">
                                ...
                                <div className="space-y-5">
                                    <FaqItem icon={<LucideIcons.Clock />} question="How long does it take?">DNS changes can be instant, but sometimes take up to 24-48 hours to update globally.</FaqItem>
                                    <FaqItem icon={<LucideIcons.Settings />} question="Where are my DNS settings?">Find them in your domain registrar's admin panel (GoDaddy, Namecheap, Cloudflare, etc.).</FaqItem>
                                </div>
                            </div>
                        </div>
                    </div>
            ...
            case 3:
                 return (
                    <div className="w-full max-w-2xl mx-auto text-center py-8 sm:py-12">
                        ...
                        <p className="text-slate-500 mb-8"><strong className="text-slate-700">{fullDomain}</strong> is now live.</p>

                        <div className="bg-white p-6 rounded-xl shadow-md border border-slate-200">
                            <h4 className="font-semibold text-slate-800">What's Next?</h4>
                            <p className="text-sm text-slate-500 mt-1 mb-4">You can assign this domain to your pages or connect another.</p>
                            <div className="flex flex-col sm:flex-row justify-center gap-3">
                                <StyledButton onClick={finishSetup} variant="primary" iconLeft={<LucideIcons.Check />} className="w-full sm:w-auto">Done</StyledButton>
                                <StyledButton onClick={connectAnother} variant="secondary" iconLeft={<LucideIcons.Plus />} className="w-full sm:w-auto">Connect Another</StyledButton>
                            </div>
                        </div>
                    </div>
                );
    ...
    const wizardButtons = () => {
        ...
        switch(currentStep) {
            case 1: return <div className="mt-10 flex justify-end"><StyledButton onClick={handleNextStep} disabled={!isStep1Complete}>Next</StyledButton></div>;
            case 2: return <div className="mt-10 flex justify-between"><StyledButton variant="secondary" onClick={handleGoBack}>Back</StyledButton><StyledButton onClick={handleVerifyAndConnect} disabled={verificationStatus === 'verifying'}>{verificationStatus === 'verifying' ? <><LucideIcons.Loader className="w-4 h-4 mr-1.5 animate-spin" /> Verifying...</> : 'Verify Connection'}</StyledButton></div>;
            default: return null;
        }
    }
    ...
    const renderDomainList = () => (
        <>
            <div className="flex flex-col gap-4 text-center md:flex-row md:justify-between md:items-center md:text-left mb-6">
                <div>
                    <h1 className="text-2xl md:text-3xl font-bold text-slate-800">Domains & URLs</h1>
                    <p className="text-sm text-slate-500 mt-1">Manage domains for your website pages and landing pages.</p>
                </div>
                <StyledButton onClick={() => setIsConnecting(true)} iconLeft={<LucideIcons.Plus/>} className="w-full md:w-auto">Connect a domain</StyledButton>
            </div>
            
            <div className="bg-white rounded-xl shadow-md border border-slate-200">
                <div className="md:hidden">
                    {domains.length > 0 ? domains.map(domain => (
                        <div key={domain.id} className="p-4 border-b last:border-b-0 border-slate-200 space-y-3">
                            <div>
                                <div className="text-xs font-medium text-slate-500">Domain</div>
                                <div className="font-medium text-slate-800">{domain.name}</div>
                            </div>
                            <div className="flex items-center justify-between gap-4">
                                <div>
                                    <div className="text-xs font-medium text-slate-500">Status</div>
                                    <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><LucideIcons.CheckCircle size={14} className="mr-1" />{domain.status}</span>
                                </div>
                                <button onClick={() => deleteDomain(domain.id)} className="text-slate-400 hover:text-red-500 p-2 rounded-md transition-colors"><LucideIcons.Trash2 size={16} /></button>
                            </div>
                            <div>
                                {domain.isPrimary ?
                                    <div className="flex items-center font-semibold text-green-700 text-sm"><LucideIcons.Star size={14} className="mr-1.5 fill-current"/> Primary for Website pages</div> :
                                    <button onClick={() => setAsPrimary(domain.id)} className="text-slate-500 hover:text-green-600 font-medium text-sm">Set as primary</button>
                                }
                            </div>
                        </div>
                    )) : (
                        <div className="text-center py-16 px-4"><LucideIcons.Globe2 size={48} className="mx-auto text-slate-300 mb-3"/><h3 className="text-lg text-slate-700 font-medium">No domains connected yet</h3><p className="text-slate-500 mt-1 text-sm">Connect a domain to build your brand.</p></div>
                    )}
                </div>
    ...
    const renderWizard = () => (
        <div className="w-full">
            <div className="mb-12 max-w-2xl mx-auto"><Stepper steps={steps} currentStep={currentStep} /></div>
            {wizardContent()}
            {wizardButtons()}
        </div>
    );
